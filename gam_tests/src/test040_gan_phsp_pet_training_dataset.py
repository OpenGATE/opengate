...
print(stats)
stats_ref = gam.read_stat_file(paths.output_ref / 'test040_train_stats.txt')
is_ok = gam.assert_stats(stats, stats_ref, 0.025)
...
tols[checked_keys.index('EventKineticEnergy')] = 0.01
tols[checked_keys.index('EventPosition_X')] = 1.4
tols[checked_keys.index('EventPosition_Y')] = 1.4
tols[checked_keys.index('EventPosition_Z')] = 1.4
...
is_ok = gam.compare_root3(ref_file, hc_file, "phase_space", "phase_space",
                          ...
                          paths.output / 'test040_train_phsp.png') and is_ok
