...
# import ../db.py
import gam_tests.sim_output_analysis as sat

# from sim_output_analysis import compareGaussParam, plot_edep

paths = gam.get_default_test_paths(__file__, "gate_test044_pbs_unFocused")
...
waterbox.translation = [-50.1 * cm, 2 * cm, 10 * cm]
...
if (not os.path.exists(ref_path / "sigma_values.txt")) or override:
    sigmasRef, musRef = gam.write_gauss_param_to_file(
        ref_path,
        planePositionsV,
        saveFig=False,
        fNamePrefix="plane",
...
if (not os.path.exists(output_path / "sigma_values.txt")) or override:
    sigmasGam, musGam = gam.write_gauss_param_to_file(
        output_path,
        planePositionsV,
        saveFig=False,
        fNamePrefix="plane",
...
for i in planePositionsV:
    ...
    """EdepColorMap = gam.create_2D_Edep_colorMap(output_path / mhd_gam)
