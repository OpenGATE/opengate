...
gam.warning('Compare acceptance angle skipped particles')
...
gam.print_test(is_ok, f'Skipped particles b1 = {b1} {r1 * 100:.2f} %  vs {reference_ratio * 100:.2f} % ')

r2 = b2 / stats.counts.event_count
is_ok = (r2 - reference_ratio) / reference_ratio < tol
gam.print_test(is_ok, f'Skipped particles b2 = {b2} {r2 * 100:.2f} %  vs {reference_ratio * 100:.2f} % ')

r3 = b3 / stats.counts.event_count
is_ok = (r3 - reference_ratio) / reference_ratio < tol
gam.print_test(is_ok, f'Skipped particles b3 = {b3} {r3 * 100:.2f} %  vs {reference_ratio * 100:.2f} % ')

# stat
gam.warning('Compare stats')
print(stats)
stats_ref = gam.read_stat_file(paths.gate_output / 'stat4.txt')
...
is_ok = gam.assert_stats(stats, stats_ref, tolerance=0.07) and is_ok
