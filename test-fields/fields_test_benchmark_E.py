#!/usr/bin/env python3
"""
Benchmark custom electric field vs Geant4 uniform electric field.
...
def _run_once_subprocess(
    ...
    result_path = out_dir / f"benchmark_E_{field_impl}_{n_histories}.json"
...
def _child_main(field_impl: str, n_histories: int, result_json: Path) -> None:
...
if __name__ == "__main__":
    ...
    rows = []
    print("\nBenchmark: custom E field vs uniform E field")
    ...
    ax.set_title("Benchmark: Custom E Field vs Uniform E Field")
    ax.legend()
    plt.grid()
    plt.tight_layout()
    plt.savefig(out_dir / "benchmark_E.png")

    csv_path = out_dir / "benchmark_fields_E.csv"
