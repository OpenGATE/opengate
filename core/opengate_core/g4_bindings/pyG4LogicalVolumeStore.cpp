...
void init_G4LogicalVolumeStore(py::module &m) {
  ...
  auto g4LogicalVolumeStore =
      py::class_<G4LogicalVolumeStore>(m, "G4LogicalVolumeStore")
          ...
          .def(
              "Get", [](G4LogicalVolumeStore *r, int i) { return (*r)[i]; },
              py::return_value_policy::reference);

  constexpr auto getVolumeArity =
      ct::functionArity(&G4LogicalVolumeStore::GetVolume);
  if constexpr (getVolumeArity == 2) {
    g4LogicalVolumeStore.def("GetVolume", &G4LogicalVolumeStore::GetVolume,
                             "name"_a, "verbose"_a = true);
