...
static bool tet_is_degenerate(const G4ThreeVector &a, const G4ThreeVector &b,
                              const G4ThreeVector &c, const G4ThreeVector &d,
                              double eps = 1e-12) {
  const auto ab = b - a;
  const auto ac = c - a;
  const auto ad = d - a;
...
void init_G4Tet(py::module &m) {
  ...
  py::class_<G4Tet, std::unique_ptr<G4Tet>>(m, "G4Tet")
      ...
      .def(
          py::init([](const std::string &name, const std::array<double, 3> &p1,
            ...
            auto *t = new G4Tet(G4String(name.c_str()),
                                ...
                                G4ThreeVector(p4[0], p4[1], p4[2]), &deg);
            // If Geant4 says degenerate, delete and throw
            if (deg) {
              delete t;
              throw std::runtime_error(
                  "G4Tet: degenerate tetrahedron (Geant4 degeneracyFlag=true)");
            }
            return t;
          }),
          py::arg("name"), py::arg("p1"), py::arg("p2"), py::arg("p3"),
          py::arg("p4"))
  ...
  m.def(
      ...
      [](const std::array<double, 3> &p1, const std::array<double, 3> &p2,
        ...
        return tet_is_degenerate(G4ThreeVector(p1[0], p1[1], p1[2]),
                                 G4ThreeVector(p2[0], p2[1], p2[2]),
                                 G4ThreeVector(p3[0], p3[1], p3[2]),
                                 G4ThreeVector(p4[0], p4[1], p4[2]), eps);
      },
