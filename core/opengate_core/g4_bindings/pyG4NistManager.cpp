...
void init_G4NistManager(py::module &m) {
  ...
  py::class_<G4NistManager>(m, "G4NistManager")
      ...
      .def(
          "FindOrBuildElement",
          [](G4NistManager *mm, const G4String &symb, G4bool isotopes = true) {
            return mm->FindOrBuildElement(symb, isotopes);
          },
          py::arg("symb"), py::arg("isotopes") = true,
          py::return_value_policy::reference_internal)
      .def(
          "FindOrBuildMaterial",
          [](G4NistManager *mm, const G4String &symb, G4bool isotopes = true,
      ...
      .def(
          "ConstructNewMaterialWeights",
          [](G4NistManager *mm, const G4String &name,
             const std::vector<G4String> &elm,
             const std::vector<G4double> &weight, G4double dens) {
