...
public:
  GatePolyhedra(
      ...
      : G4Polyhedra(name, phiStart, phiTotal, vnumSide, numZPlanes,
                    &(zPlane[0]), &(rInner[0]), &(rOuter[0])) {}
};

void init_G4Polyhedra(py::module &m) {
  py::class_<GatePolyhedra, G4VSolid,
             std::unique_ptr<GatePolyhedra, py::nodelete>>(m, "G4Polyhedra")

      .def(py::init<const G4String &, G4double, G4double, // phi start & total
                    G4int, G4int,                // nb sides, nb zplanes
                    const std::vector<G4double>, // position of z planes
                    const std::vector<G4double>, // tangent distance to inner
                                                 // surface
