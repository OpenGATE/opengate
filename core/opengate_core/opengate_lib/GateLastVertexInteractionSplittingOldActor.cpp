...
//
//
/// \file GateLastVertexInteractionSplittingActorOld.cc
/// \brief Implementation of the GateLastVertexInteractionSplittingActorOld class
...
#include "GateLastVertexSplittingPostStepDoItOld.h"
#include "GateOptnComptSplitting.h"

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

GateLastVertexInteractionSplittingActorOld::
...
G4Track *GateLastVertexInteractionSplittingActorOld::CreateComptonTrack(
...
void GateLastVertexInteractionSplittingActorOld::ComptonSplitting(
    G4Step *CurrentStep, G4Track *track, const G4Step *step,
    G4VProcess *process) {
  // Loop on process and add the secondary tracks to the current step secondary
  // vector
  ...
  if (fRussianRouletteForAngle == true) {
    G4double weightToApply = RussianRouletteForAngleSurvival(
        momentum, fVectorDirector, fMaxTheta, fSplittingFactor);
    if (weightToApply != 0) {
      gammaWeight = gammaWeight * weightToApply;
      G4Track *newTrack =
          CreateComptonTrack(gammaProcessFinalState, *track, gammaWeight);
...
void GateLastVertexInteractionSplittingActorOld::SecondariesSplitting(
    G4Step *CurrentStep, G4Track *track, const G4Step *step,
    G4VProcess *process) {
  // Loop on process and add the secondary tracks to the current step secondary
  // vector.

  // std::cout<<track->GetKineticEnergy()<<"
  // "<<track->GetDynamicParticle()->GetKineticEnergy()<<"
  // "<<track->GetMomentumDirection()<<"
  // "<<track->GetDynamicParticle()->GetMomentumDirection()<<std::endl;
  ...
  if (process->GetProcessName() == "eBrem") {
    ...
    processFinalState =
        bremProcess->GateBremPostStepDoIt::PostStepDoIt(*track, *step);
  } else {
    GateGammaEmPostStepDoIt *emProcess = (GateGammaEmPostStepDoIt *)process;
    ...
    if (track->GetTrackStatus() == 1) {
      GateplusannihilAtRestDoIt *eplusAnnihilProcess =
          (GateplusannihilAtRestDoIt *)process;

      processFinalState =
          eplusAnnihilProcess->GateplusannihilAtRestDoIt::AtRestDoIt(*track,
                                                                     *step);
    }
  }
  ...
  if (NbOfSecondaries > 0) {
    ...
    for(int i = 1; i < NbOfSecondaries;i++){
...
void GateLastVertexInteractionSplittingActorOld::ClearRememberedTracksAndSteps(
    std::map<G4int, G4TrackVector> rememberedTracks,
    std::map<G4int, std::vector<G4Step *>> rememberedSteps) {
  ...
  for (auto it = rememberedTracks.begin(); it != rememberedTracks.end(); ++it) {
    ...
    for (auto it2 = vector.begin(); it2 != vector.end(); it2++) {
      if ((std::find(trackToKill.begin(), trackToKill.end(), *it2) ==
           trackToKill.end())) {
  ...
  for (auto it = rememberedSteps.begin(); it != rememberedSteps.end(); ++it) {
    ...
    for (auto it2 = vector.begin(); it2 != vector.end(); it2++) {
      if ((std::find(stepToKill.begin(), stepToKill.end(), *it2) ==
           stepToKill.end())) {
...
void GateLastVertexInteractionSplittingActorOld::RememberLastProcessInformation(
    G4Step *step) {

  // When an interesting process to split occurs, we remember the status of the
  // track and the process at this current step some informations regarding the
  // track info have to be changed because they were update according to the
  // interaction that occured These informations are stocked as a map object,
  // binding the track ID with all the track objects and processes to split.
  // Because in some cases, if a secondary was created before an interaction
  // chain, this secondary will be track after the chain and without this
  // association, we wll loose the information about the process occuring for
  // this secondary.
  ...
  //"<<step->GetTrack()->GetTrackStatus()<<std::endl;
  ...
  if ((std::find(fListOfProcesses.begin(), fListOfProcesses.end(),
                 processName) != fListOfProcesses.end())) {
    ...
    if ((processName == "eBrem") || ((processName == "annihil") &&
                                     step->GetTrack()->GetTrackStatus() == 1)) {
      trackInformation->SetKineticEnergy(stepPoint->GetKineticEnergy() -
    ...
    if (auto search = fRememberedTracks.find(trackID);
        search != fRememberedTracks.end()) {
    ...
    if (auto search = fRememberedTracks.find(trackID);
        search == fRememberedTracks.end()) {
      if (auto search = fRememberedTracks.find(parentID);
          search != fRememberedTracks.end()) {
        if (auto it = std::find(fRememberedProcesses[parentID].begin(),
                                fRememberedProcesses[parentID].end(),
                                creatorProcessName);
            it != fRememberedProcesses[parentID].end()) {
          auto idx = it - fRememberedProcesses[parentID].begin();
          fRememberedTracks[trackID] = {
              new G4Track(*fRememberedTracks[parentID][idx])};
          fRememberedProcesses[trackID] = {fRememberedProcesses[parentID][idx]};
          fRememberedSteps[trackID] = {
              new G4Step(*fRememberedSteps[parentID][idx])};
        } else {
          fRememberedTracks[trackID] = {
              new G4Track(*fRememberedTracks[parentID][0])};
          fRememberedProcesses[trackID] = {fRememberedProcesses[parentID][0]};
          fRememberedSteps[trackID] = {
              new G4Step(*fRememberedSteps[parentID][0])};
        }
      }
    }
  }
...
void GateLastVertexInteractionSplittingActorOld::CreateNewParticleAtTheLastVertex(
    G4Step *CurrentStep, G4Track *track, const G4Step *step,
...
void GateLastVertexInteractionSplittingActorOld::
    ResetProcessesForEnteringParticles(G4Step *step) {

  // This function reset the processes and track registered to be split each
  // time a particle enters into a volume. Here a new particle incoming is
  // either a particle entering into the volume ( first pre step is a boundary
  // of a mother volume or first step trigger the actor and the vertex is not
  // either the mother volume or a the fdaughter volume) or an event generated
  // within the volume (the particle did not enter into the volume and its
  // parentID = 0). An additionnal condition is set with the trackID to ensure
  // particles crossing twice the volume (after either a compton or pair prod)
  // are not splitted.

  G4int trackID = step->GetTrack()->GetTrackID();
  if ((fEventIDOfInitialSplittedTrack != fEventID) ||
      ((fEventIDOfInitialSplittedTrack == fEventID) &&
       (trackID < fTrackIDOfInitialTrack))) {
    G4String logicalVolumeNamePreStep = "None";
    if (step->GetPreStepPoint()->GetPhysicalVolume() != 0) {
      logicalVolumeNamePreStep = step->GetPreStepPoint()
                                     ->GetPhysicalVolume()
                                     ->GetLogicalVolume()
                                     ->GetName();
    }
...
void GateLastVertexInteractionSplittingActorOld::
    PostponeFirstAnnihilationTrackIfInteraction(G4Step *step,
                                                G4String processName) {
  // In case the first gamma issued from annihilation undergoes an interaction,
  // in order to not bias the process We keep in memory the particle post step
  // state (with its secondaries) and kill the particle and its secondaries. If
  // the second photon from annihilation exiting the collimation system with an
  // interaction or is absorbed within the collimation, the particle is
  // subsequently resimulated, starting from the interaction point.
...
void GateLastVertexInteractionSplittingActorOld::
    RegenerationOfPostponedAnnihilationTrack(G4Step *step) {

  // If the second photon from annihilation suceed to exit the collimation
  // system with at least one interaction or was absorbed. Resimulation of
  // annihilation photons and its potential secondaries.

  G4TrackVector *currentSecondaries = step->GetfSecondary();
  for (int i = 0; i < fTracksToPostpone.size(); i++) {
    G4Track *trackToAdd =
        new G4Track(fTracksToPostpone[fTracksToPostpone.size() - 1 - i]);
  ...
  // Handle of case were the interaction killed the photon issued from
  // annihilation, it will not be track at the following state and the boolean
  // plus the track vector need to be reset
...
void GateLastVertexInteractionSplittingActorOld::
    HandleTrackIDIfPostponedAnnihilation(G4Step *step) {
  // The ID is to modify trackID and the processes and tracks sotcked for a
  // specific trackID associated to the postponed annihilation to respect the
  // trackID order in GEANT4. Since in this case the second photon is tracked
  // before the first one his trackID +=1. For the postponed one, he is the
  // first secondary particle of the second photon tracks, his trackID is
  // therefore equal to the second photon trackID +1 instead of the second
  // photon  trackID -1. The parentID of secondary particles are also modified
  // because they are used in the rememberlastprocess function At last the value
  // associated to a specific trackID in the process and track map are modified
  // according to the new trackID.

  if (fSuspendForAnnihil) {
    if (step->GetTrack()->GetTrackID() ==
        fTracksToPostpone[0].GetTrackID() - 1) {
      fRememberedProcesses[step->GetTrack()->GetTrackID()] =
          fRememberedProcesses[step->GetTrack()->GetTrackID() + 1];
      fRememberedProcesses.erase(step->GetTrack()->GetTrackID() + 1);
      fRememberedSteps[step->GetTrack()->GetTrackID()] =
          fRememberedSteps[step->GetTrack()->GetTrackID() + 1];
      fRememberedSteps.erase(step->GetTrack()->GetTrackID() + 1);
      fRememberedTracks[step->GetTrack()->GetTrackID()] =
          fRememberedTracks[step->GetTrack()->GetTrackID() + 1];
...
void GateLastVertexInteractionSplittingActorOld::BeginOfRunAction(
    const G4Run *run) {
...
void GateLastVertexInteractionSplittingActorOld::BeginOfEventAction(
    const G4Event *event) {
...
void GateLastVertexInteractionSplittingActorOld::PreUserTrackingAction(
    const G4Track *track) {
  fIsFirstStep = true;
}

void GateLastVertexInteractionSplittingActorOld::SteppingAction(G4Step *step) {
  ...
  // std::cout<<particleName<<"     "<<fEventID<<"
  // "<<step->GetTotalEnergyDeposit()<<"
  // "<<step->GetPreStepPoint()->GetKineticEnergy()<<"
  // "<<step->GetPreStepPoint()->GetMomentumDirection()<<step->GetTrack()->GetTrackID()<<"
  // "<<step->GetTrack()->GetWeight()<<" "<<step->GetTrack()->GetParentID()<<"
  // "<<fTrackIDOfSplittedTrack<<std::endl;;
  ...
  if ((step->GetTrack()->GetWeight() < fWeightOfEnteringParticle) &&
    ...
    if (particleName != "e+"){
      if (std::find(fListOfProcesses.begin(), fListOfProcesses.end(), processName) != fListOfProcesses.end()){
        ...
        for (int i = 0; i < step->GetfSecondary()->size(); i++){
          G4Track* track =(*(step->GetfSecondary()))[i];
    ...
    if ((step->GetTrack()->GetTrackStatus() == 2) ||
        (step->GetTrack()->GetTrackStatus() == 3)) {

      /*
      delete fTrackToSplit;
      delete fStepToSplit;
      fTrackToSplit = new
      G4Track(*fRememberedTracks[fTrackIDOfSplittedTrack].back()); fStepToSplit
      = new G4Step(*fRememberedSteps[fTrackIDOfSplittedTrack].back());
      */

      CreateNewParticleAtTheLastVertex(step, fTrackToSplit, fStepToSplit,
                                       fProcessToSplit);
    }

    if (((step->GetTrack()->GetTrackStatus() != 2) &&
         (step->GetTrack()->GetTrackStatus() != 3)) &&
        (std::find(fListOfVolumeAncestor.begin(), fListOfVolumeAncestor.end(),
      ...
      if (fSplitCounter < fWeightOfEnteringParticle) {
        ...
        delete fStepToSplit;
        fTrackToSplit = new
        G4Track(*fRememberedTracks[fTrackIDOfSplittedTrack].back());
        fStepToSplit = new
        G4Step(*fRememberedSteps[fTrackIDOfSplittedTrack].back());
        */
        CreateNewParticleAtTheLastVertex(step, fTrackToSplit, fStepToSplit,
                                         fProcessToSplit);
  ...
  if (step->GetTrack()->GetWeight() == fWeightOfEnteringParticle) {
    ...
    if (auto search = fRememberedProcesses.find(trackID);
        search != fRememberedProcesses.end()) {
      process = fRememberedProcesses[trackID].back();
    }
    // std::cout<<step->GetTrack()->GetVertexPosition()<<"
    // "<<step->GetTrack()->GetParticleDefinition()->GetParticleName()<<"
    // "<<fSuspendForAnnihil<<"    "<<process<<"
    // "<<step->GetTrack()->GetWeight()<<" "<<step->GetTrack()->GetParentID()<<"
    // "<<step->GetTrack()->GetTrackID()<<"    "<<creatorProcessName<<"
    // "<<step->GetTrack()->GetTrackStatus()<<"
    // "<<step->GetPreStepPoint()->GetKineticEnergy()<<"    "
    // <<step->GetPostStepPoint()->GetKineticEnergy()<<"
    // "<<step->GetPreStepPoint()->GetPosition()<<"
    // "<<step->GetPostStepPoint()->GetPosition()<<std::endl;

    /*
    if ((!fSuspendForAnnihil) && (creatorProcessName == "annihil") &&
    (fTrackIDOfSplittedTrack == step->GetTrack()->GetParentID()) && (fEventID ==
    fEventIDOfSplittedTrack))
    {
      step->GetTrack()->SetTrackStatus(fKillTrackAndSecondaries);
    }
    */
    ...
    if ((particleName == "e-") &&
        (step->GetTrack()->GetWeight() == fWeightOfEnteringParticle) &&
        (creatorProcessName == "conv") &&
        (fTrackIDOfSplittedTrack == step->GetTrack()->GetParentID()) &&
        (fEventID == fEventIDOfSplittedTrack)) {
      step->GetTrack()->SetTrackStatus(fStopAndKill);
    }

    /*
    if ((!fSuspendForAnnihil) && (process != "annihil") && (creatorProcessName
    == "annihil") && (step->GetTrack()->GetTrackStatus() != 3))
    {
      G4int parentID = step->GetTrack()->GetParentID();
      if (auto search = fRememberedProcesses.find(parentID); search !=
    fRememberedProcesses.end())
      {
    ...
    // If the first annihilation photon exit the collimation and the process to
    split is annihilation
    // We kill the second photon, because the annihilation will generate both
    the photons.
  ...
  */
    ...
    G4String logicalVolumeNamePostStep = "None";
    G4ThreeVector particleMomentum = step->GetPostStepPoint()->GetMomentumDirection();
    if (step->GetPostStepPoint()->GetPhysicalVolume() != 0)
      logicalVolumeNamePostStep = step->GetPostStepPoint()
                                      ->GetPhysicalVolume()
                                      ->GetLogicalVolume()
    ...
    if (((step->GetTrack()->GetTrackStatus() != 2) &&
         (step->GetTrack()->GetTrackStatus() != 3)) &&
        (std::find(fListOfVolumeAncestor.begin(), fListOfVolumeAncestor.end(),
                   logicalVolumeNamePostStep) != fListOfVolumeAncestor.end())&& (particleMomentum[2] < 0)) {
      ...
      if (std::find(fListOfProcesses.begin(), fListOfProcesses.end(),
                    fProcessToSplit) != fListOfProcesses.end()) {
        ...
        // If the process t split is the gamma issued from compton interaction,
        // the electron primary generated have to be killed given that electron
        // will be regenerated
        ...
        // If the process to split is the annihilation, the second photon,
        // postponed or not, have to be killed the reset of the postpone is
        // performed here, whereas the kill of the next annihilation photon, if
        // not postponed is realised at the beginning of the step tracking.
        /*
        ...
        */
        // Handle of pecularities 3 :  If the positron which created one or more
        // brem photons exits all the brems photons will be killed before their
        // tracking, and the conv processes will then be replayed
  ...
  /*
  if ((fSuspendForAnnihil) && ((step->GetTrack()->GetTrackStatus() == 1) ||
  (step->GetTrack()->GetTrackStatus() == 2)))
  {
...
void GateLastVertexInteractionSplittingActorOld::PostUserTrackingAction(
    const G4Track *track) {}
