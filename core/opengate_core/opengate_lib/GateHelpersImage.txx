...
void ImageAddValue(typename ImageType::Pointer image,
  ...
  auto v = image->GetPixel(
      index); // FIXME maybe 2 x FastComputeOffset can be spared ?
  image->SetPixel(index, v + value);
}

template <class ImageType>
void AttachImageToVolume(typename ImageType::Pointer image,
                         std::string volumeName, G4ThreeVector image_offset,
  ...
  // volume rotation is to rotate the image according to the orientation of the
  // volume For example, in a crystal, the depth could be Z, or X or Y
  // dimension, while the image is always with depth=Z
  ...
  for (auto i = 0; i < 3; i++)
    origin[i] =
        size[i] * spacing[i] / -2.0 + spacing[i] / 2.0 + image_offset[i];
