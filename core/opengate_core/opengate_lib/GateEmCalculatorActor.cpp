...
//-----------------------------------------------------------------------------
/// Construct
GateEmCalculatorActor::GateEmCalculatorActor(py::dict &user_info) : GateVActor(user_info, false) {
...
//   double CSDArange = 0;
  ...
  for(size_t j=0;j<mEnergies.size();j++){
      ...
      EmDEDX = emcalc->ComputeElectronicDEDX(mEnergy, particle_definition, mat, cut)*gram/(MeV*cm2*density); 
