...
void GateSourceManager::StartMasterThread() {
    ...
    // progress bar
    if (fProgressBarFlag) {
        if (G4Threading::IsMultithreadedApplication() && G4Threading::G4GetThreadId() != 0) return;
...
void GateSourceManager::InitializeProgressBar() {
    ...
    if (G4Threading::IsMultithreadedApplication() && G4Threading::G4GetThreadId() != 0) return;
    auto &l = fThreadLocalData.Get();
    l.fProgressBar = new ProgressBar{option::BarWidth{50},
                                     option::Start{""},
                                     option::Fill{"■"},
                                     option::Lead{"■"},
                                     option::End{""},
...
void GateSourceManager::ComputeExpectedNumberOfEvents() {
    ...
    for (auto *source: fSources) {
        fExpectedNumberOfEvents += source->GetExpectedNumberOfEvents(fSimulationTimes);
    }
