...
public:
  ...
  void BeginOfRunActionMasterThread(int run_id) override {
    PYBIND11_OVERLOAD(void, GateBeamQualityActor, BeginOfRunActionMasterThread, run_id);
  }

  int EndOfRunActionMasterThread(int run_id) override {
    PYBIND11_OVERLOAD(int, GateBeamQualityActor, EndOfRunActionMasterThread, run_id);
  }
};

void init_GateBeamQualityActor(py::module &m) {
  py::class_<GateBeamQualityActor, PyGateBeamQualityActor,
             std::unique_ptr<GateBeamQualityActor, py::nodelete>, GateWeightedEdepActor>(
      m, "GateBeamQualityActor")
      .def(py::init<py::dict &>())
      ...
      .def_readwrite("cpp_numerator_alpha_image", &GateBeamQualityActor::cpp_numerator_image)
      .def_readwrite("cpp_numerator_beta_image", &GateBeamQualityActor::cpp_second_numerator_image)
     ...
     // .def_readwrite("cpp_nucleus_dose_image", &GateBeamQualityActor::cpp_nucleus_dose_image)
      .def_readwrite("NbOfEvent", &GateBeamQualityActor::NbOfEvent)
      .def("GetPhysicalVolumeName", &GateBeamQualityActor::GetPhysicalVolumeName)
      .def("SetPhysicalVolumeName", &GateBeamQualityActor::SetPhysicalVolumeName);
