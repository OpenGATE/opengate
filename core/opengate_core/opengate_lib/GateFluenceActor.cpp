...
void GateFluenceActor::BeginOfRunActionMasterThread(int run_id) {
  ...
  AttachImageToVolume<Image3DType>(cpp_counts_image, fPhysicalVolumeName,
                                   fTranslation);
  if (fSecondaries) {
    AttachImageToVolume<Image3DType>(cpp_counts_compt_image,
  ...
  if (fEnergyFlag) {
    AttachImageToVolume<Image3DType>(cpp_energy_image, fPhysicalVolumeName,
                                     fTranslation);
    if (fSecondaries) {
      AttachImageToVolume<Image3DType>(cpp_energy_compt_image,
                                       fPhysicalVolumeName, fTranslation);
      AttachImageToVolume<Image3DType>(cpp_energy_rayl_image,
                                       fPhysicalVolumeName, fTranslation);
      AttachImageToVolume<Image3DType>(cpp_energy_sec_image,
                                       fPhysicalVolumeName, fTranslation);
      AttachImageToVolume<Image3DType>(cpp_energy_prim_image,
                                       fPhysicalVolumeName, fTranslation);
    }
  }
  NbOfEvent = 0;
      ...
      }
      {
        ...
        if (fEnergyFlag) {
          ...
          if (fSecondaries) {
            if ((lastProcessName == "compt") ||
                (creatorProcessName == "biasWrapper(compt)")) {
              ImageAddValue<Image3DType>(cpp_energy_compt_image, index,
                                         energy * w);
            }
            if ((lastProcessName == "Rayl") ||
                (creatorProcessName == "biasWrapper(Rayl)")) {
              ImageAddValue<Image3DType>(cpp_energy_rayl_image, index,
                                         energy * w);
            }
            if (((lastProcessName == "Rayl") ||
                 (creatorProcessName == "biasWrapper(Rayl)")) ||
                ((lastProcessName == "compt") ||
                 (creatorProcessName == "biasWrapper(compt)"))) {
              ImageAddValue<Image3DType>(cpp_energy_sec_image, index,
                                         energy * w);
            } else {
              ImageAddValue<Image3DType>(cpp_energy_prim_image, index,
                                         energy * w);
            }
          }
        }
          ...
          if (fSecondaries) {
            if ((lastProcessName == "compt") ||
              ...
              ScoreSquaredValue(fThreadLocalDataComptEnergy.Get(),
                                cpp_energy_squared_compt_image, energy * w,
                                event_id, index);
            }
            if ((lastProcessName == "Rayl") ||
                (creatorProcessName == "biasWrapper(Rayl)")) {
              ScoreSquaredValue(fThreadLocalDataRaylEnergy.Get(),
                                cpp_energy_squared_rayl_image, energy * w,
                                event_id, index);
            }
            if (((lastProcessName == "Rayl") ||
                 (creatorProcessName == "biasWrapper(Rayl)")) ||
              ...
              ScoreSquaredValue(fThreadLocalDataSecEnergy.Get(),
                                cpp_energy_squared_sec_image, energy * w,
                                event_id, index);
            } else {
              ScoreSquaredValue(fThreadLocalDataPrimEnergy.Get(),
                                cpp_energy_squared_prim_image, energy * w,
                                event_id, index);
            }
          }
        }
...
void GateFluenceActor::EndOfEventAction(const G4Event *event) {
  if (fCountsSquaredFlag) {
    ...
    if (fSecondaries) {
      FlushSquaredValues(fThreadLocalDataComptCounts.Get(),
                         cpp_counts_squared_compt_image);
      FlushSquaredValues(fThreadLocalDataRaylCounts.Get(),
                         cpp_counts_squared_rayl_image);
      FlushSquaredValues(fThreadLocalDataSecCounts.Get(),
                         cpp_counts_squared_sec_image);
      FlushSquaredValues(fThreadLocalDataPrimCounts.Get(),
                         cpp_counts_squared_prim_image);
    }
  }
  if (fEnergySquaredFlag) {
    FlushSquaredValues(fThreadLocalDataEnergy.Get(), cpp_energy_squared_image);
    if (fSecondaries) {
      FlushSquaredValues(fThreadLocalDataComptEnergy.Get(),
                         cpp_energy_squared_compt_image);
      FlushSquaredValues(fThreadLocalDataRaylEnergy.Get(),
                         cpp_energy_squared_rayl_image);
      FlushSquaredValues(fThreadLocalDataSecEnergy.Get(),
                         cpp_energy_squared_sec_image);
      FlushSquaredValues(fThreadLocalDataPrimEnergy.Get(),
                         cpp_energy_squared_prim_image);
    }
  }
}
