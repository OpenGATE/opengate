...
public:
  ...
  G4bool Notify(const char *originOfException, const char *exceptionCode,
    ...
    PYBIND11_OVERLOAD(G4bool, GateExceptionHandler, Notify, originOfException,
                      exceptionCode, severity, description);
  }
};
...
void init_GateExceptionHandler(py::module &m) {
  ...
  py::class_<GateExceptionHandler, PyGateExceptionHandler>(
      m, "GateExceptionHandler")
      .def(py::init())
