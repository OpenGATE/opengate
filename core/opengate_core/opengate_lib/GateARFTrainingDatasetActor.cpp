...
GateARFTrainingDatasetActor::GateARFTrainingDatasetActor(py::dict &user_info)
    ...
    fEnergyWindowsActor = dynamic_cast<GateHitsEnergyWindowsActor *>(GateActorManager::GetActor(fInputActorName));
...
void GateARFTrainingDatasetActor::StartSimulationAction() {
    fHits = GateHitsCollectionManager::GetInstance()->NewHitsCollection(fHitsCollectionName);
