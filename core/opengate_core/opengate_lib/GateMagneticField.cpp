...
GateMagneticField::GateMagneticField(
    ...
    double deltaChordMM
) : G4MagneticField(), GateField(solid, std::move(translations), std::move(rotations), deltaChordMM), m_inner(inner)
{
...
) const
{
  ...
  // get the global coordinates of the point and the transform of the containing placement
  const G4ThreeVector worldPoint(Point[0], Point[1], Point[2]);

  // get the local coordinates of the point in the containing placement, and the transform of that placement
  const G4AffineTransform *transform = nullptr;
  const G4ThreeVector localPoint =
      findContainingPlacement(worldPoint, transform);
  const G4double localPos[4] = {localPoint.x(), localPoint.y(), localPoint.z(), Point[3]};
  ...
  const G4ThreeVector worldB = rotateToWorld({localB[0], localB[1], localB[2]}, *transform);
