...
G4Mutex SetWorkerEndRunMutexSec = G4MUTEX_INITIALIZER;
G4Mutex SetPixelMutexSec = G4MUTEX_INITIALIZER;
...
void GateDoseActorSecondariesFromPhotons::ActorInitialize() {
...
void GateDoseActorSecondariesFromPhotons::BeginOfRunAction(const G4Run *run) {
...
void GateDoseActorSecondariesFromPhotons::SteppingAction(G4Step *step) {
  ...
  if (isInside) {
    ...
    if (fcpImageForThreadsFlag) {
      ...
      G4AutoLock mutex(&SetPixelMutexSec);
...
// void GateDoseActorSecondariesFromPhotons::EndSimulationAction() {
...
double GateDoseActorSecondariesFromPhotons::ComputeMeanUncertainty() {
  G4AutoLock mutex(&ComputeUncertaintyMutexSec);
  itk::ImageRegionIterator<Image3DType> edep_iterator3D(
      cpp_edep_image, cpp_edep_image->GetLargestPossibleRegion());
  double mean_unc = 0.0;
  int n_voxel_unc = 0;
...
void GateDoseActorSecondariesFromPhotons::ComputeSquareImage() {
  G4AutoLock mutex(&SetWorkerEndRunMutexSec);
  threadLocalT &data = fThreadLocalData.Get();
