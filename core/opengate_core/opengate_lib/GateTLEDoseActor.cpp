...
void GateTLEDoseActor::PreUserTrackingAction(const G4Track *track) {
  ...
  // if the particle is a gamma, we associate a map which will associate the gamma TID with the number of secondaries created 
  // when the particle is in TLE mode
  ...
  //if the particle is not a gamma, we want to associate a secondary boolean to allow or not the energy deposition.
  //- If it's a direct secondary of the gamma with the PID inside, which have to not deposit its energy, the boolean is set to false
  //- If it's a direct secondary, but the number of remaining secondaries to track is 0, it means that this secondary was not created by a
  //TLE gamma, and the boolean is set to false
  //- If it's an indirect secondary, created by a secondary generated by a gamma, we do nothing, since the boolean was already fixed.
  // If it's a primary or a secondary generated from a primary which is not a gamma, the boolean is set to False at the beginning of the event
