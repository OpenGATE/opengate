...
void GateHitsEnergyWindowsActor::BeginOfRunAction(const G4Run *run) {
    ...
    if (run->GetRunID() == 0) {
        ...
        for (auto *hc: fChannelHitsCollections) {
            ...
            auto *f = new GateHitsAttributesFiller(fInputHitsCollection, hc, hc->GetHitAttributeNames());
...
void GateHitsEnergyWindowsActor::ApplyThreshold(size_t i, double min, double max) {
    auto &l = fThreadLocalData.Get();
...
// Called every time a Run ends
void GateHitsEnergyWindowsActor::EndOfSimulationWorkerAction(const G4Run * /*run*/) {
    for (auto *hc: fChannelHitsCollections)
        hc->Write();
}
