...
void GateVoxelizedPromptGammaTLEActor::SteppingAction(G4Step *step) {
  ...
  if (prot) {
    if (!energy) {
      GetStepVoxelPosition<ImageType>(step, "random", cpp_tof_proton_image,
                                      position, isInside, index);
    } else {
      GetStepVoxelPosition<ImageType>(step, "random", cpp_E_proton_image,
                                      position, isInside, index);
    }
  } else {
    if (!energy) {
      GetStepVoxelPosition<ImageType>(step, "random", cpp_tof_neutron_image,
                                      position, isInside, index);
    } else {
      GetStepVoxelPosition<ImageType>(step, "random", cpp_E_neutron_image,
                                      position, isInside, index);
    }
  }
  ...
  for (size_t i = 0; i < sec->size(); i++) {
    ...
    if (!energy) {
      ...
      G4double rho =
          mat->GetDensity() / (CLHEP::g / CLHEP::cm3); // Convert to g/cm3
      if (prot) {
