...
// Convert the index to a 4D index
...
G4double projectileEnergy = preE;
if (postE != 0) {
  const G4double &randomenergy = G4UniformRand();
  projectileEnergy = postE + randomenergy * (preE - postE); // MeV
}
// Get the energy bin index
G4int binE = static_cast<int>(
  projectileEnergy / (energyrange / energybins)); // Always the left bin
if (binE >= energybins) {
binE = energybins; // last bin = overflow
}
...
if ((fProtonTimeFlag) ||
  ...
  const G4double &randomtime = G4UniformRand();
  const G4double &pretime = step->GetPreStepPoint()->GetGlobalTime() - T0;   // ns
  const G4double &posttime = step->GetPostStepPoint()->GetGlobalTime() - T0; // ns
...
void GateVoxelizedPromptGammaTLEActor::SetVector(py::array_t<double> vect_p, 
  ...
  fProtonVector = std::vector<double>(vect_p.data(), vect_p.data() + vect_p.size());
  fNeutronVector = std::vector<double>(vect_n.data(), vect_n.data() + vect_n.size());
}