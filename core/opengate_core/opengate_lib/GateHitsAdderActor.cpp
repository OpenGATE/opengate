...
GateHitsAdderActor::GateHitsAdderActor(py::dict &user_info)
    ...
    if (fPolicy == AdderPolicy::Error) {
        ...
        oss
            << "Error in GateHitsAdderActor: unknown policy. Must be EnergyWinnerPosition or EnergyWeightedCentroidPosition"
...
void GateHitsAdderActor::InitializeComputation() {
    ...
    l.fHitsAttributeFiller = new GateHitsAttributesFiller(fInputHitsCollection,
