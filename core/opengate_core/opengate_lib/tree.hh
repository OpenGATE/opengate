...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator
tree<T, tree_node_allocator>::insert(sibling_iterator position, T &&x) {
  tree_node *tmp =
      std::allocator_traits<decltype(alloc_)>::allocate(alloc_, 1, 0);
  std::allocator_traits<decltype(alloc_)>::construct(alloc_, tmp);
  std::swap(tmp->data, x); // Move semantics
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::fixed_depth_iterator::swap(
    fixed_depth_iterator &first, fixed_depth_iterator &second) {
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::sibling_iterator::swap(
    sibling_iterator &first, sibling_iterator &second) {
  std::swap(first.node, second.node);
  std::swap(first.skip_current_children_, second.skip_current_children_);
