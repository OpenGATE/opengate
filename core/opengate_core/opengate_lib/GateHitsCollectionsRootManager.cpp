...
GateHitsCollectionsRootManager *GateHitsCollectionsRootManager::fInstance = nullptr;

GateHitsCollectionsRootManager *GateHitsCollectionsRootManager::GetInstance() {
    if (fInstance == nullptr) fInstance = new GateHitsCollectionsRootManager();
    return fInstance;
}
...
void GateHitsCollectionsRootManager::OpenFile(int tupleId, std::string filename) {
...
void GateHitsCollectionsRootManager::CreateNtupleColumn(int tupleId, GateVHitAttribute *att) {
    auto *ram = G4RootAnalysisManager::Instance();
    int att_id = -1;
