...
GateHitAttributeManager *GateHitAttributeManager::GetInstance() {
    if (fInstance == nullptr) fInstance = new GateHitAttributeManager();
    return fInstance;
}
...
std::vector<std::string> GateHitAttributeManager::GetAvailableHitAttributeNames() {
...
void GateHitAttributeManager::DefineHitAttribute(std::string name, char type,
                                                const GateVHitAttribute::ProcessHitsFunctionType &f) {
    GateVHitAttribute *att = nullptr;
    if (type == 'D') att = new GateTHitAttribute<double>(name);
    if (type == 'I') att = new GateTHitAttribute<int>(name);
    if (type == 'S') att = new GateTHitAttribute<std::string>(name);
    if (type == '3') att = new GateTHitAttribute<G4ThreeVector>(name);
    if (type == 'U') att = new GateTHitAttribute<GateUniqueVolumeID::Pointer>(name);
    if (att == nullptr) {
        std::ostringstream oss;
...
GateVHitAttribute *GateHitAttributeManager::CopyHitAttribute(GateVHitAttribute *att) {
    ...
    if (att->GetHitAttributeType() == 'U') {
        a = new GateTHitAttribute<GateUniqueVolumeID::Pointer>(att->GetHitAttributeName());
    }
    if (a != nullptr) {
