...
GateImageNestedParameterisation::GateImageNestedParameterisation() : G4VNestedParameterisation() {
    // Create the image pointer
    // size and allocation will be performed on the py side
    cpp_image = ImageType::New();
}

void GateImageNestedParameterisation::initialize_image() {
...
void GateImageNestedParameterisation::initialize_material(std::vector<std::string> materials) {
    fMaterials.resize(0);
    for (auto s:materials) {
        auto m = G4Material::GetMaterial(s);
        if (m == nullptr) {
            std::cout << "GateImageNestedParameterisation: cannot find the material " << s << std::endl;
            exit(0);
        }
...
G4Material *GateImageNestedParameterisation::ComputeMaterial(G4VPhysicalVolume * /*currentVol*/,
                                                            const G4int repNo,
...
G4int GateImageNestedParameterisation::GetNumberOfMaterials() const {
    return fMaterials.size();
}

G4Material *GateImageNestedParameterisation::GetMaterial(G4int idx) const {
    return fMaterials[idx];
}

void GateImageNestedParameterisation::ComputeTransformation(const G4int no,
