...
GateOptrForceCollision::GateOptrForceCollision(py::dict &user_info)
    : G4VBiasingOperator("forceCollisionActor"), GateVActor(user_info, false),
      fForceCollisionModelID(
          G4PhysicsModelCatalog::GetModelID("model_GenBiasForceCollision")),
      fCurrentTrack(nullptr), fCurrentTrackData(nullptr),
      fInitialTrackWeight(-1.0), fSetup(true) {
  fSharedForceInteractionOperation =
      new GateOptnForceCommonTruncatedExp("SharedForceInteraction");
...
GateOptrForceCollision::~GateOptrForceCollision() {
  for (std::map<const G4BiasingProcessInterface *,
                GateOptnForceFreeFlight *>::iterator it =
           fFreeFlightOperations.begin();
       it != fFreeFlightOperations.end(); it++)
    delete (*it).second;
...
void GateOptrForceCollision::ConfigureForWorker() {
  // -- start by remembering processes under biasing, and create needed biasing
  // operations:
  if (fSetup) {
    const G4ProcessManager *processManager =
        fParticleToBias->GetProcessManager();
    const G4BiasingProcessSharedData *interfaceProcessSharedData =
        G4BiasingProcessInterface::GetSharedData(processManager);
    if (interfaceProcessSharedData) // -- sharedData tested, as is can happen a
                                    // user attaches an operator
                                    // -- to a volume but without defining
                                    // BiasingProcessInterface processes.
    {
      for (size_t i = 0;
           i <
           (interfaceProcessSharedData->GetPhysicsBiasingProcessInterfaces())
               .size();
           i++) {
        const G4BiasingProcessInterface *wrapperProcess =
            (interfaceProcessSharedData
                 ->GetPhysicsBiasingProcessInterfaces())[i];
        G4String operationName =
            "FreeFlight-" +
            wrapperProcess->GetWrappedProcess()->GetProcessName();
        fFreeFlightOperations[wrapperProcess] =
            new GateOptnForceFreeFlight(operationName);
      }
    }
    fSetup = false;
  }
...
G4VBiasingOperation *GateOptrForceCollision::ProposeOccurenceBiasingOperation(
  ...
  if (fCurrentTrackData == nullptr) {
    // ... and if the track has no aux. track data, it means its biasing is not
    // started yet (note that cloning is to happen first):
    fCurrentTrackData =
        (GateOptrForceCollisionTrackData *)(track->GetAuxiliaryTrackInformation(
            fForceCollisionModelID));
    if (fCurrentTrackData == nullptr)
      return nullptr;
  }
  ...
  if (fCurrentTrackData->fForceCollisionState ==
      ForceCollisionState::toBeFreeFlight) {
    GateOptnForceFreeFlight *operation = fFreeFlightOperations[callingProcess];
    if (callingProcess->GetWrappedProcess()->GetCurrentInteractionLength() <
        DBL_MAX / 10.) {
      // -- the initial track weight will be restored only by the first DoIt
      // free flight:
  ...
  if (fCurrentTrackData->fForceCollisionState ==
    ...
    // -- Remember if this calling process is the first of the physics wrapper
    // in
    ...
    if (isFirstPhysGPIL) {
      // -- first step of cloned track, initialize the forced interaction
      // operation:
      if (track->GetCurrentStepNumber() == 1)
        fSharedForceInteractionOperation->Initialize(track);
      else {
        if (fSharedForceInteractionOperation->GetInitialMomentum() !=
            track->GetMomentum()) {
          // -- means that some other physics process, not under control of the
          // forced interaction operation,
          // -- has occured, need to re-initialize the operation as distance to
          // boundary has changed.
          // -- [ Note the re-initialization is only possible for a Markovian
          // law. ]
          fSharedForceInteractionOperation->Initialize(track);
        } else {
          // -- means that some other non-physics process (biasing or not, like
          // step limit), has occured,
          // -- but track conserves its momentum direction, only need to reduced
    ...
    // -- [*all processes*] Sanity check : it may happen in limit cases that
    // distance to
    // -- out is zero, weight would be infinite in this case: abort forced
    // interaction
    // -- and abandon biasing.
    ...
    // -- [* first process*] collect cross-sections and sample force law to
    // determine interaction length
    // -- and winning process:
    if (isFirstPhysGPIL) {
      // -- collect cross-sections:
      // -- ( Remember that the first of the G4BiasingProcessInterface triggers
      // the update
      // --   of these cross-sections )
      const G4BiasingProcessSharedData *sharedData =
          callingProcess->GetSharedData();
      for (size_t i = 0;
           i < (sharedData->GetPhysicsBiasingProcessInterfaces()).size(); i++) {
        const G4BiasingProcessInterface *wrapperProcess =
            (sharedData->GetPhysicsBiasingProcessInterfaces())[i];
        G4double interactionLength =
            wrapperProcess->GetWrappedProcess()->GetCurrentInteractionLength();
        // -- keep only well defined cross-sections, other processes are
        // ignored. These are not pathological cases
        // -- but cases where a threhold effect par example (eg pair creation)
        // may be at play. (**!**)
        if (interactionLength < DBL_MAX / 10.)
          fSharedForceInteractionOperation->AddCrossSection(
              wrapperProcess->GetWrappedProcess(), 1.0 / interactionLength);
      }
    ...
    // -- [*all processes*] Send operation for processes with well defined XS
    // (see "**!**" above):
  ...
  } // -- end of "if ( fCurrentTrackData->fForceCollisionState ==
    // ForceCollisionState::toBeForced )"
...
G4VBiasingOperation *GateOptrForceCollision::ProposeNonPhysicsBiasingOperation(
    const G4Track *track,
  ...
  // -- A "cloning" is done:
  // --  - the primary will be forced flight under a zero weight up to volume
  // exit,
  ...
  if (track->GetStep()->GetPreStepPoint()->GetStepStatus() ==
      fGeomBoundary) // -- §§§ extent to case of a track shoot on the boundary ?
  {
    // -- check that track is free of undergoing biasing scheme ( no biasing
    // data, or no active active )
    ...
    if (fCurrentTrackData != nullptr) {
      if (fCurrentTrackData->IsFreeFromBiasing()) {
        // -- takes "ownership" (some track data created before, left free,
        // reuse it):
        fCurrentTrackData->fForceCollisionOperator = this;
      } else {
        // §§§ Would something be really wrong in this case ? Could this be that
        // a process made a zero step ?
      }
    } else {
      fCurrentTrackData = new GateOptrForceCollisionTrackData(this);
      track->SetAuxiliaryTrackInformation(fForceCollisionModelID,
                                          fCurrentTrackData);
    }
...
G4VBiasingOperation *GateOptrForceCollision::ProposeFinalStateBiasingOperation(
    const G4Track *, const G4BiasingProcessInterface *callingProcess) {
  // -- Propose at final state generation the same operation which was proposed
  // at GPIL level,
  // -- (which is either the force free flight or the force interaction
  // operation).
  ...
  if (fCurrentTrackData != nullptr) {
    if (!fCurrentTrackData->IsFreeFromBiasing()) {
      if ((fCurrentTrack->GetTrackStatus() == fStopAndKill) ||
        ...
        ed << "Current track deleted while under biasing by " << GetName()
           << ". Will result in inconsistencies.";
        G4Exception(" GateOptrForceCollision::EndTracking()", "BIAS.GEN.18",
                    JustWarning, ed);
      }
    }
  }
...
void GateOptrForceCollision::OperationApplied(
    const G4BiasingProcessInterface *callingProcess, G4BiasingAppliedCase BAC,
    G4VBiasingOperation *operationApplied, const G4VParticleChange *) {
  ...
  if (fCurrentTrackData->fForceCollisionState ==
    ...
    fCurrentTrackData->fForceCollisionState =
        ForceCollisionState::toBeFreeFlight;
    auto cloneData = new GateOptrForceCollisionTrackData(this);
    cloneData->fForceCollisionState = ForceCollisionState::toBeForced;
    fCloningOperation->GetCloneTrack()->SetAuxiliaryTrackInformation(
        fForceCollisionModelID, cloneData);
  } else if (fCurrentTrackData->fForceCollisionState ==
             ForceCollisionState::toBeFreeFlight) {
