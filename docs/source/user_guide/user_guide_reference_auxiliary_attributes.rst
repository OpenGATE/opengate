...
Exposes the current Track Length Estimator (TLE) track mode as an integer
runtime attribute.
...
This attribute owns the TLE track-state policy and genealogy propagation logic.
It is primarily intended for use by :class:`~.opengate.actors.doseactors.TLEDoseActor`
when that actor is configured with ``tle_state_mode = "auxiliary"``.

These integer values are part of the current public user-facing meaning of the
attribute and can therefore also be consumed elsewhere, for example:

- in a :class:`~.opengate.actors.digitizers.PhaseSpaceActor` attribute list
- in a generic filter built with :class:`~.opengate.actors.filters.GateFilterBuilder`
...
.. code-block:: python
   ...
   F = gate.GateFilterBuilder()
   phsp.filter = F(tle_mode.name) == 1

Reference
~~~~~~~~~

.. autoclass:: opengate.auxiliary_attributes.TLETrackModeAttribute
