...
.. code:: python
    ...
    source.direction.histogram_theta_weight = [1]
    source.direction.histogram_theta_angle = [80 * deg, 100 * deg]
    source.direction.histogram_phi_weight = [0.3, 0.5, 1, 0.5, 0.3]
...
if their position and direction make them intersect the given volume. This
...
Predefined energy spectra for beta+
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
