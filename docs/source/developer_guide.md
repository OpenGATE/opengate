...
To **develop**, you need 1) to compile and create the first `gam_g4` module and 2) pip install the second (Python only) `gam_gate` module.
...
This repository contains C++ source code that maps some (not all!) Geant4 classes into one single Python module. It also contains additional C++ classes that extends Geant4 functionalities (also mapped to Python). At the end of the compilation process a single Python module is available, named `gam-g4` and is ready to use from the Python side.

The source files are divided into two folders: `g4_bindings` and `gam_lib`. The first contains pure Geant4 Python bindings allow to expose in Python a (small) part of Geant4 classes and functions. The bindings are done with the [pybind11](https://github.com/pybind/pybind11) library. The second folder contains specific gam functionalities.
...
We recommend to look at an example (e.g. `GamDoseActor` ). The main concept is to write a cpp class that will act during the simulation, and to manage all users options and parameters from the python side. Geant4 messengers are no longer used. There is a mechanism, explained below, in order to convert python options to cpp options. Writing a new actor (a kind of scorer) involves 4 steps.

#### 1 - Create a c++ class `GamMyActor`

In a file `GamMyActor.cpp`, Within the `gam_g4/gam_g4/gam_lib/` folder. This class should inherit from `GamVActor` and implement the virtual functions that are triggered by Geant4 engine when Run, Event, Track or Step start or end. Here are the list of functions:
...
This is done by creating a cpp file `pyGamMyActor.cpp`, look at `pyGamDoseActor.cpp` as example. This file contains one unique function (called `init_GamMyActor`) that indicates what are the member functions or variables that should be exposed to python. Most of the time, there are very few elements, or none. The constructor is however mandatory. The exposed functions could then be called from the python side.

This function must be declared in the `gam_g4/gam_g4.cpp` file (defined at the beginning and called within the main function).

#### 3 - Create a python file `MyActor.py` with the corresponding python class

This file must contain one class  `MyActor`, inheriting from 2 classes: 1) the cpp class you just wrote in the previous step, `g4.GamMyActor` and 2) the python `gam.ActorBase` class. The  `MyActor` class must implement:
...
```python
class DoseActor(g4.GamMyActor, gam.ActorBase):
    ...
    def __init__(self, user_info):
        ...
        g4.GamMyActor.__init__(self, user_info.__dict__)

    def initialize(self):
        gam.ActorBase.initialize(self)

    def StartSimulationAction(self):
        g4.GamMyActor.StartSimulationAction(self)
        print('optional')

    def EndSimulationAction(self):
        g4.GamMyActor.EndSimulationAction(self)

    # etc
```

Finally, the new actor must be declared in the `gam_gate/actor/helpers_actor.py` file.
