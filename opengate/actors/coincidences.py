...
def process_chunk(
    keys, chunk, coincidences, time_window, minDistanceXY, maxDistanceZ, policy
):
    singles = chunk
    coinc_events = []
...
def calculate_sector_difference(coincidence, i):
    ...
    x1, y1, z1 = (
        coincidence["PostPosition_X1"][i],
        coincidence["PostPosition_Y1"][i],
        coincidence["PostPosition_Z1"][i],
    )
    x2, y2, z2 = (
        coincidence["PostPosition_X2"][i],
        coincidence["PostPosition_Y2"][i],
        coincidence["PostPosition_Z2"][i],
    )
...
def take_winner_of_goods(coincidences, time_window, minDistanceXY, maxDistanceZ):
    ...
    for bucket, indices in time_buckets.items():
        ...
        if len(indices) > 0:
            highest_energy_index = max(
                indices,
                key=lambda idx: coincidences["TotalEnergyDeposit1"][idx]
...
def take_if_only_one_good(coincidences, time_window, minDistanceXY, maxDistanceZ):
    ...
    for bucket, indices in time_buckets.items():
        ...
        if good_found:
            ids_to_keep.extend(
                indices
            )  # If one good pair is found, add all indices in this bucket
    return filter_coincidences(coincidences, ids_to_keep)
...
def take_winner_if_is_good(coincidences, time_window, minDistanceXY, maxDistanceZ):
    ...
    for bucket, indices in time_buckets.items():
        if len(indices) > 0:
            ...
            if is_good_pair(
                coincidences, highest_energy_index, minDistanceXY, maxDistanceZ
            ):
                ids_to_keep.append(highest_energy_index)
