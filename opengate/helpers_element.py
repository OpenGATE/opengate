...
def get_element_class(element_type, type_name):
    ...
    if not elements:
        gate.fatal(f'Error, element_type={element_type} is   unknown. Use Volume, Source or Actor.')
    for e in elements:
        # check the class has type_name
        if not hasattr(e, 'type_name'):
            gate.fatal(f'Error, the class {e.__name__} *must* have a static attribute called "type_name"')
        # is the type the one we are looking ?
        if e.type_name == type_name:
            return e
    s = [x.type_name for x in elements]
    gate.fatal(f'Error {element_type}: the type "{type_name}" is unknown. Known types are {s}')
