...
def info_timing(i):
    s = f'[ {gate.g4_best_unit(i[0], "Time")}; {gate.g4_best_unit(i[1], "Time")}]'
    return s


def assert_run_timing(run_timing_intervals):
    if len(run_timing_intervals) < 0:
        gate.fatal(f'run_timing_interval must be an array '
    ...
    for i in run_timing_intervals:
        if len(i) != 2:
            gate.fatal(f'This run_timing_interval must have '
                      f'exactly one start and one end time, while it is: {i}')
        if i[0] > i[1]:
            gate.fatal(f'Start time must be lower or equal than end time: {i}')
        if i[0] < previous_end:
            gate.fatal(f'Start time must be larger or equal than previous end time: {i}, previous: {previous_end}')
        previous_end = i[1]


def info_run_timing(sim: gate.Simulation):
