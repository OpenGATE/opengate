...
class voxelised_dose():
    ...
    def __init__(self,img,**kwargs):
        x, y, z, arrayD, voxel_size, nb_voxel, offset_voxel = self.open_itk_dose_image(img)
        img_dose = itk.imread(img)
        self.x = x
        self.y = y
        self.z = z
        self.dose = arrayD
        self.information = Box({"spacing":voxel_size,"number_of_voxels":nb_voxel,"offset":offset_voxel,"errors" : False,"img":img_dose})
        ...
        for err_img in l_err_img:
            if os.path.exists(err_img):
                x, y, z, err_arrayD, voxel_size, nb_voxel, offset_voxel = self.open_itk_dose_image(err_img)
...
for pname in folder_list:
    ...
    path_output = f"../output"
    if tle :
        folder_list = sorted(glob.glob(f"{path_output}/tle-stats_file*",recursive=True))
    else :
        folder_list = sorted(glob.glob(f"{path_output}/tle-stats_file*", recursive=True))
    print(folder_list)
    ...
    print(isocenter)
    if tle :
        img_dose_name_list = sorted(glob.glob("../output/*tle-norm-dose*.mhd",recursive=True))
    else :
        img_dose_name_list = sorted(glob.glob("../output/*norm-dose*.mhd", recursive=True))
    ...
    cax = ax1[0].imshow(np.rot90(CT.dose[:,idx,:]), extent=[x.min(), x.max(),z.max(), z.min()], origin='lower', cmap='gist_gray',aspect='equal')
    cax2 = ax1[0].imshow(np.rot90(MC_dose.dose[:,idx,:]), extent=[x.min(), x.max(),z.max(), z.min()], origin='lower', cmap='jet',norm= norm, aspect='equal',alpha = 0.6)
    cbar = fig1.colorbar(cax2,ax = ax1[0])
    cbar.set_label(label = r'\textbf{Dose[\%]}',fontsize=16)


#
    cax = ax1[1].imshow(np.rot90(CT.dose[:,idx,:]), extent=[x.min(), x.max(),z.max(), z.min()], origin='lower', cmap='gist_gray',aspect='equal')
    cax2 = ax1[1].imshow(np.rot90(TPS.dose[:,idx,:]), extent=[x.min(), x.max(),z.max(), z.min()], origin='lower', cmap='jet',norm= norm, aspect='equal',alpha = 0.6)
