...
def isocenter_area_norm_factor(array, isocenter_idx, size):
    sum_voxel_area = np.sum(
        array[
            isocenter_idx[0] - int(size / 2) : isocenter_idx[0] + int(size / 2) + 1,
            isocenter_idx[1] - int(size / 2) : isocenter_idx[1] + int(size / 2) + 1,
            isocenter_idx[2] - int(size / 2) : isocenter_idx[2] + int(size / 2) + 1,
        ]
    )
...
class MC_img:
    ...
    def __init__(self, name, array, header, **kwargs):
        ...
        if type(header) == list:
            self.header = Box(
                {"offset": header[0], "dim": header[1], "spacing": header[2]}
            )
        else:
            self.header = header
        self.img_to_norm = []
...
def norm_img(path):
    ...
    for the_path in folder_list:
        ...
        if os.path.isfile(name_tle_MC_img):
            array_MC, img_MC, headers_MC = read_mhd_img(
                name_tle_MC_img, return_img_dim=True
            )
        ...
        if os.path.isfile(name_MC_img):
            array_MC, img_MC, headers_MC = read_mhd_img(
                name_MC_img, return_img_dim=True
            )
