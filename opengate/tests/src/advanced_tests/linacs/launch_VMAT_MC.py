...
@click.option('--dcm_file', default='./data/Patients_IGR/P1/RTPlan_P1.dcm', help='Pathname to the .dcm filenames')
@click.option('--arc_id', default=None ,help='ID of the beam sequence to reproduce. If None, all the beam sequences will be simulated')
@click.option('--path_img', default='./data/Patients_IGR/P1/',help='img to put in the simulation')
...
@click.option('--path_phsp', default="/home/mjacquet/Documents/phsp/" ,help='path of the phsp_source')
@click.option('--phsp_name', default="phsp_6.4MeV_sx_1.15_sy_0.8.root" ,help='phsp source name')
@click.option('--vis', default=False ,help='Use the simulation visualisation')
@click.option('--mode', default=1 ,help='Define how actors are applied : 0 for normal, 1 for TLE')
@click.option('--tle_type', default=0,help='tle type to apply a cutoff : 0 for an energy threshold (energy),1 for the max range of sec part threshold (max range) or 2 for the average range of sec part threshold (average range)')
@click.option('--tle_threshold', default=1.6,help='threshold on ekin (MeV) or range (mm) depending on the tle_type')
@click.option('--ratio', default=1,help='string information in case of ratio mix between no int on int')
@click.option('--shielding', default=True,help="boolean to put a shielding around the LINAC")
@click.option('--lead_thickness', default=4,help="the thickness in cm of the lead part above the jaw")
@click.option('--json_input', default=True,help='use a json to define all the input')
@click.option('--json_name', default="./data/header.json",help='json input name if they are provided via a json file')
def launch_simulation(nt,nb_part,dcm_file,arc_id,path_img,img,output_name,path_output,path_phsp,phsp_name,mode,vis,tle_type,tle_threshold,ratio,shielding,lead_thickness,json_input,json_name):
    nb_jobs = 1
    if json_input:
        f = open(f"./{json_name}")
    ...
    sim,rt_plan_params = t.init_simulation(dcm_file,arc_id,path_img,img,mode,vis=vis,tle_type=tle_type, tle_threshold=tle_threshold,shielding=shielding,lead_thickness=lead_thickness)
    if vis :
        sim.visu = True
    ...
    if "dose" in actors_list:
        ...
        dose_actor.dose_uncertainty.output_filename = "./" +output_name[:-4] +  '-dose-uncertainty.mhd'
        dose_actor.edep_uncertainty.output_filename = "./"  +output_name[:-4] +  '-edep-uncertainty.mhd'
        dose_actor.dose_squared.output_filename = "./"  +output_name[:-4] + '-dose-squared.mhd'
        dose_actor.edep_squared.output_filename = "./" +output_name[:-4] + '-edep-squared.mhd'
    ...
    if "tle_dose_actor" in actors_list:
        ...
        tle_dose_actor.dose_uncertainty.output_filename = "./" + output_name[:-4] + '-tle-dose-uncertainty.mhd'
        tle_dose_actor.edep_uncertainty.output_filename = "./" + output_name[:-4] + '-tle-edep-uncertainty.mhd'
        tle_dose_actor.dose_squared.output_filename = "./" + output_name[:-4] + '-tle-dose-squared.mhd'
        tle_dose_actor.edep_squared.output_filename = "./" + output_name[:-4] + '-tle-edep-squared.mhd'
