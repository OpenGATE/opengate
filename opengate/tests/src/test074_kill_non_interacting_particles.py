...
def test074_test(entry_data, exit_data_1,exit_data_2):
    ...
    while i < len(evt_ID_entry_data):
        if j < len(exit_data_1["EventID"])  and evt_ID_entry_data[i] == exit_data_1["EventID"][j]:
            ...
            if (j < len(exit_data_1["EventID"]) -1) and (exit_data_1["EventID"][j] == exit_data_1["EventID"][j+1]) :
                i = i-1
            j+=1
        i+=1
    liste_ekin  =np.asarray(liste_ekin)
    print("Number of tracks to kill =", len(liste_ekin))
    print("Number of killed tracks =",(len(exit_data_1["EventID"]) - len(exit_data_2["EventID"])))


    return len(liste_ekin) == (len(exit_data_1["EventID"]) - len(exit_data_2["EventID"]))
...
if __name__ == "__main__":
    ...
    tungsten_leaves.color = [0.9,0.,0.4,0.8]
    ...
    liste_phase_space_name = [entry_phase_space.name,exit_phase_space_1.name,exit_phase_space_2.name]
