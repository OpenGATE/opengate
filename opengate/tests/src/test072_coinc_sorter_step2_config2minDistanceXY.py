...
if __name__ == "__main__":
    ...
    paths = utility.get_default_test_paths(
        __file__, output_folder="test072"
    )
    ...
    coincidences = coincidences_sorter(
        singles_tree, time_window, policy, minDistanceXY, maxDistanceZ, chunk_size=1000000
    )
    ...
    print(f"Stat comparison {nc} (Gate10) vs. {nc_ref} (Gate9.4): {stat_diff}, tolerance {tolerance_stat}")
    
    
    both_posX = np.concatenate((coincidences["PostPosition_X1"], coincidences["PostPosition_X2"]))
    ref_both_posX = np.concatenate((ref_coincidences["globalPosX1"], ref_coincidences["globalPosX2"]))

    both_energy = np.concatenate((coincidences["TotalEnergyDeposit1"], coincidences["TotalEnergyDeposit2"]))
    ref_both_energy = np.concatenate((ref_coincidences["energy1"], ref_coincidences["energy2"]))

    # Calculate Wasserstein distance for comparison
    distance_posX = wasserstein_distance(
        both_posX, ref_both_posX
    )
    tolerance_posX = 0.8 
    print(f"Wasserstein distance on X : {distance_posX}, tolerence {tolerance_posX}")
    
    distance_energy = wasserstein_distance(
        both_energy, ref_both_energy
    )
    tolerance_energy = 0.005
    print(f"Wasserstein distance on energy : {distance_energy}, tolerence {tolerance_energy}")
    

    is_ok = (stat_diff < tolerance_stat and
             distance_posX < tolerance_posX and
             distance_energy < tolerance_energy)
