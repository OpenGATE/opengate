...
if __name__ == "__main__":
    paths = utility.get_default_test_paths(
        __file__, "gate_test041_dose_actor_dose_to_other_material", "test041"
    )
    ...
    # pmma slab
    pmma_slab_insert = sim.add_volume("Box", "pmma_slab_insert")
    pmma_slab_insert.mother = phantom_off.name
    pmma_slab_insert.size = [2 * mm, 20 * mm, 20 * mm]
    pmma_slab_insert.translation = [43 * mm, 0, 0]
    pmma_slab_insert.material = "PMMA"
    pmma_slab_insert.color = [0, 0, 1, 1]
    ...
    # each actor is attached to a different volume and/or scores in pmma/the local material
    ...
    dose_actor_IDD_d2w.score_in = "PMMA"
    dose_actors.append(dose_actor_IDD_d2w)

    dose_actor_pmma_slab_insert_d = sim.add_actor("DoseActor", "IDD_pmmaSlab_d")
    dose_actor_pmma_slab_insert_d.attached_to = pmma_slab_insert
    dose_actors.append(dose_actor_pmma_slab_insert_d)

    dose_actor_pmma_slab_insert_d2w = sim.add_actor("DoseActor", "IDD_pmmaSlab_d2w")
    dose_actor_pmma_slab_insert_d2w.attached_to = pmma_slab_insert
    dose_actor_pmma_slab_insert_d2w.score_in = "PMMA"
    dose_actors.append(dose_actor_pmma_slab_insert_d2w)
    ...
    dose_actor_entranceRegiont_d2w.score_in = "PMMA"
    ...
    mSPR_40MeV = 1.235294  # from PSTAR NIST tables, Feb 2026
    gate.exception.warning(
        "Test ratio: dose / dose_to_pmma in geometry with material: PMMA"
    )
    is_ok = utility.assert_images_ratio(
        1.00,
        dose_actor_pmma_slab_insert_d.dose.get_output_path(),
        dose_actor_pmma_slab_insert_d2w.dose.get_output_path(),
        abs_tolerance=0.05,
    )

    gate.exception.warning(
        "Test ratio: dose / dose_to_pmma in geometry with material: G4_Si"
    )
    is_ok = (
