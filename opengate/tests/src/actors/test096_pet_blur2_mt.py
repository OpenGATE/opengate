...
if __name__ == "__main__":
    paths = utility.get_default_test_paths(
        __file__, gate_folder="", output_folder="test096_pet_blur2"
    )
    ...
    pet = vereos.add_pet(sim, "pet", nb_module=4)
    pet.translation = [1 * cm, 4 * cm, -2 * cm]
    pet.rotation = Rotation.from_euler("yz", (40, 3), degrees=True).as_matrix()
    ...
    hits_actor, blur_actor = add_test_digitizer(
        sim, crystal, "output_blur2_keep_in_solid_limits.root"
    )
    ...
    # Save volumes
    filename = paths.output / "castor_config_blur2.json"
    ...
    vrml_filename = output_path / "castor_config_blur2.vrml"
