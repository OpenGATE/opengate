...
if __name__ == "__main__":
    ...
    fluence_actor.output_filename = "test099_processes_ffaa.mhd"
    ...
    ff.kill_interacting_in_volumes = [fluence_plane.name]
    ff.compton_splitting_factor = 10  # the value must have no effect
    ff.rayleigh_splitting_factor = 100  # the value must have no effect
    ff.max_compton_level = 100  # count everything
    ...
    for process in processes:
        for type in types:
            ...
            img_fluence = img_opening(
                output_path / f"test099_processes_ffaa_{type}_{process}.mhd"
            )
            img_squared_fluence = img_opening(
                output_path / f"test099_processes_ffaa_{type}_squared_{process}.mhd"
            )
            img_uncertainty_fluence = img_opening(
                output_path / f"test099_processes_ffaa_{type}_uncertainty_{process}.mhd"
            )
