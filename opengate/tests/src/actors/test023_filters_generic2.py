...
if __name__ == "__main__":
    ...
    phsp.output_filename = f"{sim_name}_2.root"
    ...
    ti = tree.arrays(phsp.attributes)["KineticEnergy"]
    tmin = np.min(ti)
    tmax = np.max(ti)
    b = 300 * keV < tmin and 400 * keV > tmax
    utility.print_test(b, f"Energy = min={tmin / keV} max={tmax / keV}")
    is_ok = b and is_ok

    ti = tree.arrays(phsp.attributes)["ParticleName"]
    b = True
    for t in ti:
        if t != "gamma":
            b = False
    utility.print_test(b, f"Particle Name gamma? ")
    is_ok = b and is_ok
