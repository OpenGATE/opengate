...
def create_electron_flash_simulation(paths, passive_collimation, fantom):
    ...
    if passive_collimation in ["app40", "app100"]:
        app_end = fun.build_passive_collimation(sim, passive_collimation, center_z=ef_end, material_colors=fun.material_colors)
        dim_x, dim_y, dim_z = 250 * mm, 250 * mm, 60 * mm
    elif passive_collimation == "mb_40_slit_11":
        mb_end = fun.build_passive_collimation(sim, "app40", center_z=ef_end,
                                             material_colors=fun.material_colors)
        app_end = fun.build_passive_collimation(sim, passive_collimation, center_z=mb_end,
                                             material_colors=fun.material_colors)
        dim_x, dim_y, dim_z = 100*mm, 100*mm, 20*mm
    elif passive_collimation == "shaper40":
        shaper_end = fun.build_passive_collimation(sim, "app40", center_z=ef_end, material_colors=fun.material_colors)
        app_end, (leaf1, leaf2, leaf3, leaf4) = fun.build_passive_collimation(sim, passive_collimation, center_z=shaper_end,
                                                                                   material_colors=fun.material_colors)
    ...
    #=====================================================
    if fantom == "Waterbox":
        dosephantom = fun.build_dosephantombox(sim, "Waterbox", "Water", center_z=app_end+dim_z/2,dimension_x=dim_x, dimension_y=dim_y, dimension_z=dim_z, material_colors=fun.material_colors)
        ...
        phsp_plane = fun.build_dosephantombox(sim, "Phasespace_plane", "Air", center_z=app_end+dim_z/2,dimension_x=dim_x, dimension_y=dim_y, dimension_z=dim_z, material_colors=fun.material_colors)
