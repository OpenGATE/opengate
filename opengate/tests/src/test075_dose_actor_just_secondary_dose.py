...
def test075_2(img_arr_1, img_arr_2, tol=0.01):
    ...
    if (
        np.abs(np.sum(img_arr_1) - np.sum(img_arr_2)) / np.sum(img_arr_2) < tol
        and np.sum(img_arr_2) != 0
    ):
        return True
    else:
        return False
...
if __name__ == "__main__":
    ...
    img_dose = itk.imread(
        paths.output / "test075_normal_dose_actor_for_secondary-edep.mhd"
    )
    arr_dose = itk.GetArrayFromImage(img_dose)

    img_dose_sec = itk.imread(
        paths.output / "test075_normal_dose_sec_actor_for_secondary-edep.mhd"
    )
