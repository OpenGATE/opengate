...
def test099(df,l_att):
    ...
    df_selec = df[(nb_processes_layer_0 <= 1) & (nb_processes_layer_1 <= 1) & (nb_processes_tot ==2)]
    ...
    print(f"Difference between recorded event using the process defined step and the last occuring process : {np.sum(process_recorded - last_process_int)}")
