...
def is_ok_test096(rootfile_1,rootfile_2,time_intervals,nb_run,nb_part):
    ...
    for i in range(nb_run):
        tmp_time = time_1[(time_1>= time_intervals[i][0]) & (time_1< time_intervals[i][1])]
        ...
        if err_time - 4 * err_time / err_count<= theo_err<= err_time + 4 * err_time / err_count:
            count_true_source_1 += 1

        print(f"Theoretical number of particle in run_{i}:",nb_part)
        print(f"Experimental number of particle in run_{i}: {len(tmp_time)} +- {err_count}")
        print(f"Theoretical std.dev in run_{i}: {theo_err/10**9} ns")
        print(f"Experimental std.dev in run_{i}: {err_time/10**9} ns +- {err_time / err_count/10**9}")

    count_true_source_2 = 0
    print("")
    print("For n source :")
    for i in range(nb_run):
        tmp_time = time_2[(time_2>= time_intervals[i][0]) & (time_2< time_intervals[i][1])]
...
def add_source(n,type,z_dir,i):
    Bq= gate.g4_units.Bq
    source = sim.add_source("GenericSource", f"mysource_{type}_{i}")
    source.energy.mono = 150 * MeV
    source.particle = "gamma"
    source.position.type = "disc"
    source.position.radius = 5 * nm
...
def add_phsp_source(n,type,plane_name):
    Bq= gate.g4_units.Bq
    source = sim.add_source("PhaseSpaceSource", f"myphspsource_{type}")
...
if __name__ == "__main__":
    paths = utility.get_default_test_paths(
        __file__, "gate_test096", "test096"
    )

    # create the simulation
    for simu_ID in range(2):
        sim = gate.Simulation()
        ui = sim.user_info
        ...
        l_phsp = []
        l_names_src = ["plan_src_1", "plan_src_2"]
        l_src = []
        l_plans = []
        for i, name in enumerate(l_names_src):
        ...
        if simu_ID == 0:
            for i, name in enumerate(l_names_src):
                ...
                ta.output_filename = f"test096_src_{name}.root"
                l_src.append(ta)

        if simu_ID == 0:
            add_source(7*nb_part, "n", 1,0)
            add_source(7*nb_part, "n", -1,1)

        if simu_ID == 1:
            src1 =add_phsp_source(nb_part, "activity","plan_src_1")
            src1.phsp_file = paths.output/ f"test096_src_plan_src_1.root"
            src2 = add_phsp_source(np.zeros(nb_run) +nb_part, "n", "plan_src_2")
            src2.phsp_file = paths.output/ f"test096_src_plan_src_2.root"
