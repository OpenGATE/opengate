#!/usr/bin/env python3
"""
Test 099 - field direction follows volume rotation.

Two boxes share one UniformMagneticField with B along local +Y (3 T).
box_a is unrotated, box_b is rotated 90° about Z.

Checks:
  - box_a: deflection in -X, no Y deflection
  - box_b: deflection in -Y, no X deflection
  - box_a deflection in -X is similar to box_b deflection in -Y
"""
...
# Expected sagitta for a 10 MeV proton in By over BOX_SIZE depth.
...
if __name__ == "__main__":
    ...
    phsp_a.output_filename = "phsp_a.root"
    ...
    phsp_b.output_filename = "phsp_b.root"
    phsp_b.steps_to_store = "exiting"
    phsp_b.root_output.write_to_disk = True

    sim.run()

    df_a = uproot.open(str(paths.output / "phsp_a.root"))["phsp_a;1"].arrays(
        library="pd"
    )
    df_b = uproot.open(str(paths.output / "phsp_b.root"))["phsp_b;1"].arrays(
        library="pd"
    )
    ...
    # Positions in the frame of each box centre
    ...
    # box_a: deflected in -X, no Y deflection
    ...
    # box_b: B_world = -X → deflects in -Y, no X deflection
    ...
    # box_a deflection in -X is similar to box_b deflection in -Y
    ok_a_x_similar_b_y = np.abs(xa - yb) < TOL_DEFLECTED
    print(
        f"box_a deflection in -X is similar to box_b deflection in -Y: {ok_a_x_similar_b_y}"
    )

    is_ok = ok_a_x and ok_a_y and ok_b_y and ok_b_x and ok_a_x_similar_b_y
