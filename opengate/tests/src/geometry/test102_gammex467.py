...
if __name__ == "__main__":
    ...
    # Change world size
    world = sim.world
    world.size = [100.0 * cm, 100.0 * cm, 100.0 * cm]

    # Add the Gammex 467 phantom
    gammex467_phantom = gammex467.add_gammex467_phantom(sim)

    volume_labels, image = voxelize_geometry(
        sim, extent=gammex467_phantom, spacing=[1.0 * mm, 1.0 * mm, 1.0 * mm]
    )
    write_voxelized_geometry(sim, volume_labels, image, paths.output / "gammex467.mhd")

    # run
    sim.run()

    output = itk.GetArrayFromImage(itk.imread(paths.output / "gammex467.mhd"))
    reference = itk.GetArrayFromImage(itk.imread(paths.output_ref / "gammex467.mhd"))
