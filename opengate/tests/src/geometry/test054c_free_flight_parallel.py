...
if __name__ == "__main__":
    ...
    mean_weight = np.mean(weights_at_boundary) if len(weights_at_boundary) > 0 else 0.0

    print(
        f"Steps arriving at excluded volume boundary (Z=50mm): {len(weights_at_boundary)}"
    )
    print(f"Mean Track Weight at boundary: {mean_weight:.4f} (Expected: 0.0 < W < 1.0)")
    ...
    # Final Test Status
    utility.test_ok(is_weight_ok and is_edep_ok)
