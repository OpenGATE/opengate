...
if __name__ == "__main__":
    # paths
    paths = utl.get_default_test_paths(
        __file__, "gate_test075_siemens_cios_alpha", "test075"
    )
    ...
    exp_file_path = (
        current_dir.parent.parent / "contrib" / "carm" / "experimental_values.json"
    )
    ...
    for pos, sim_w, exp_w in zip(
        exp_positions_trimmed, sim_weights_interp_trimmed, exp_weights_trimmed
    ):
