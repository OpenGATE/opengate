#!/usr/bin/env python3
"""
Test 099 - MappedMagneticField on a repeated volume.

Same scenario as test099_fields_repeated_placements, but both fields are
MappedMagneticField.
"""
...
if __name__ == "__main__":
    ...
    # 2x2x2 grid covering the full single-box local extent.
    half = BOX_SIZE / 2
    corners_single = list(itertools.product([-half, half], repeat=3))
    mat_single = np.array([[x, y, z, 0.0, By, 0.0] for x, y, z in corners_single])

    field_single = fields.MappedMagneticField(name="B_single")
    field_single.field_matrix = mat_single
    field_single.interpolation = "trilinear"
    box_single.add_field(field_single)

    src_single = sim.add_source("GenericSource", "src_single")
    src_single.particle = "proton"
    src_single.number_of_primaries = 1
    ...
    phsp_single.output_filename = "phsp_mapped_repeated_single.root"
    ...
    # 2x2x2 grid covering the slab local extent: BOX_SIZE x BOX_SIZE x SLAB_SIZE.
    hxy = BOX_SIZE / 2
    hz = SLAB_SIZE / 2
    corners_slab = list(itertools.product([-hxy, hxy], [-hxy, hxy], [-hz, hz]))
    mat_multi = np.array([[x, y, z, 0.0, By, 0.0] for x, y, z in corners_slab])

    field_multi = fields.MappedMagneticField(name="B_multi")
    field_multi.field_matrix = mat_multi
    field_multi.interpolation = "trilinear"
    box_multi.add_field(field_multi)

    src_multi = sim.add_source("GenericSource", "src_multi")
    src_multi.particle = "proton"
    src_multi.number_of_primaries = 1
    ...
    phsp_multi.output_filename = "phsp_mapped_repeated_multi.root"
    phsp_multi.steps_to_store = "exiting"
    phsp_multi.root_output.write_to_disk = True

    sim.run()

    df_single = uproot.open(str(paths.output / "phsp_mapped_repeated_single.root"))[
        "phsp_single;1"
    ].arrays(library="pd")
    df_multi = uproot.open(str(paths.output / "phsp_mapped_repeated_multi.root"))[
        "phsp_multi;1"
    ].arrays(library="pd")
