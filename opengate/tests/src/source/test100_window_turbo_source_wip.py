...
def compare_profiles(ref, test, tolerance=8.0, threshold=0.01, fig_name=None):
    ...
    # Ignore low-stat tails, otherwise relative differences explode.
    mask = ref > threshold * ref.max()
    ref_m = ref[mask]
    test_m = test[mask]

    # Compare shape, not raw activity.
    ref_n = ref_m / ref_m.sum()
    test_n = test_m / test_m.sum()

    sad = np.abs(ref_n - test_n).sum() * 100
    is_ok = sad < tolerance
    utility.print_test(
