...
def opengate_run(
    ...
    sim, ct, vpg_actor, source = simulation(
        output, File_name, job_id, number_of_particles, visu, verbose, actor
    )
...
def itk_merge(
    ...
    try:
        ...
        file_pattern = os.path.join(
            f"/home/vguittet/Documents/G10/stage1.5/output/{output}/{File_name}_*.nii.gz"
        )
        ...
        for file_path in file_list:
            ...
            init = itk.imread(
                path.output / f"{File_name}_{actor_list[0]}_1_{type_list[0]}"
            )
            ...
            for type_name in type_list:
                for actor_name in actor_list:
                    ...
                    for i in range(count):
                        file_path = (
                            path.output
                            / f"{File_name}_{actor_name}_{i + 1}_{type_name}"
                        )
