...
def validation_test(arr_ref, arr_data, nb_split):
    ...
    bool_weight = False
    print("Weight mean is equal to",weight_data, "and need to be equal to",1/nb_split)
    ...
    keys = ["KineticEnergy", "PreDirection_X","PreDirection_Y","PreDirection_Z","PrePosition_X","PrePosition_Y"]
    ...
    for key in keys :
        ...
        print(key,"mean ref value:", np.round(mean_ref,3),"+-",np.round(std_err_ref,3))
        print(key,"mean data value:", np.round(mean_data,3),"+-",np.round(std_err_data,3))


        if (mean_data - 4 * np.sqrt(std_err_data**2 + std_err_ref**2) > mean_ref) or  (mean_data + 4 * np.sqrt(std_err_data**2 + std_err_ref**2) <mean_ref):
            bool_distrib = False
