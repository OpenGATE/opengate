...
# activity
activity = 1e4 * Bq / ui.number_of_threads
...
sc = gate_spect.add_digitizer_deposited_energy(sim, crystal_name)
...
arf.output = "/Users/nkrah/test043_arf_training_dataset_deposited_energy.root"
# arf.output = paths.output / "test043_arf_training_dataset_deposited_energy.root"
arf.hits_collection_name = sc.name
arf.hits_collection_attribute = "TotalEnergyDeposit"
...
print(f"Nb of skipped particles {skip}  {(skip / stat.counts.event_count) * 100:.2f}%")
...
checked_keys = [
    ...
    {"k1": "Edep", "k2": "Edep", "tol": 0.002, "scaling": 1},
]
is_ok = (
    gate.compare_root2(
        paths.output_ref / "test043_arf_training_dataset_deposited_energy.root",
        arf.output,
        "ARF (training)",
        "ARF (training)",
        checked_keys,
        paths.output / "test043_training_dataset_deposited_energy.png",
        n_tol=14,
    )
    and is_ok
)
