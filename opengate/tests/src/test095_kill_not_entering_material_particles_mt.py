...
def validation_test(arr_data, nb_part):
    ...
    theoretical_number_of_gamma = (
        nb_part * (2 * np.pi * (1 - np.cos(np.pi / 4))) / (4 * np.pi)
    )
    print(
        "Number of detected particles: {} +/- {} ".format(
            nb_detected_gamma, np.round(err_nb_detected_gamma)
        )
    )
    print(
        "Theoretical number of detected particles: {}".format(
            np.round(theoretical_number_of_gamma)
        )
    )
    if (
        nb_detected_gamma - 4 * err_nb_detected_gamma
        < theoretical_number_of_gamma
        < nb_detected_gamma + 4 * err_nb_detected_gamma
    ):
        return True
...
if __name__ == "__main__":
    paths = utility.get_default_test_paths(
        __file__,
        "gate_test095_kill_according_crossed_material",
        output_folder="kill_according_crossed_material",
    )
    ...
    kill_if_no_galactic = sim.add_actor(
        "KillParticlesNotCrossingMaterialsActor", "kill_by_mat_actor"
    )
