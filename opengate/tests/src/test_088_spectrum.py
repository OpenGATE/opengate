...
from multijobs2 import DEFAULT_OUTPUT, DEFAULT_FILE_NAME, DEFAULT_NUMBER_OF_PARTICLES, DEFAULT_ACTOR

simu, ct, vpg_actor, source = simulation(output = DEFAULT_OUTPUT, File_name = DEFAULT_FILE_NAME, 
                 job_id = 0, number_of_particles = DEFAULT_NUMBER_OF_PARTICLES, 
                 visu = False, verbose = False, actor = DEFAULT_ACTOR)
...
for type_name in feature:
    ...
    treated_array = np.zeros((250, array.shape[1], array.shape[2], array.shape[3]))  # Initialize the treated array
    for x in range(array.shape[3]):
        for y in range(array.shape[2]):
            ...
            # Get the Hounsfield Unit (HU) value at the current voxel
                ...
                UH = UH_array[X, Y, Z]
                histo_E = array[:, z, y, x]  # Get the energy histogram for the current voxel
                Gamma_m = gamma_proton[UH] if type_name == "prot_e.nii.gz" else gamma_neutron[UH]
                spectrum = np.zeros(250)  # Initialize the spectrum for the current voxel
