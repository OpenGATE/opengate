...
def main():
    ...
    geometries = ["Box"]  # , "Tubs"] # FIXME
    grid_sizes = [[1, 1, 200]]  # , [1, 100, 200], [50, 100, 200]] # FIXME
    res = 1 * mm
    spacing = [res, res, res]
    ...
    for geom in geometries:
        ...
        for grid_size in grid_sizes:
            ...
            m = add_volume(sim, "Box", f"mother_{i}", [box_side] * 3, transl=transl)
            v = add_volume(
...
def check_dose_grid_geometry(dose_mhd_path, dose_actor):
    ...
    shape = data.shape
    print(f"     dose_actor test img ", img.GetLargestPossibleRegion().GetSize())
    print(f"     dose_actor test arr", shape)
    spacing = img.GetSpacing()
    print(f"     dose_actor size", dose_actor.size)
    print(f"     dose_actor size", np.flip(dose_actor.size))
    shape_ref = tuple(np.flip(dose_actor.size))
    print(f"     dose_actor shape_ref", shape_ref)
    spacing_ref = dose_actor.spacing

    ok = shape == shape_ref
    gate.print_test(ok, f"    {shape=} same than {shape_ref=} ? {ok}")
