...
def delete_run_manager_if_needed(sim):
    # if sys.platform == 'darwin':
        ...
        gate.warning('WARNING, we need to delete G4RunManager, otherwise, GIL bug (seg fault)')
...
def compare_root2(root1, root2, branch1, branch2, keys, img_filename, n_tol=3):
    ...
    is_ok = gate.print_test(diff < n_tol, f'Difference: {hits1_n} {hits2_n} {diff:.2f}% (tol = {n_tol:.2f})')
...
def compare_root3(root1, root2, branch1, branch2, keys1, keys2, tols, scalings1, scalings2, img, hits_tol=6):
    ...
    print(f'Current tree:   {os.path.basename(root2)} n={hits2_n}')
    diff = gate.rel_diff(float(hits1_n), float(hits2_n))
    is_ok = gate.print_test(diff < hits_tol, f'Difference: {hits1_n} {hits2_n} {diff:.2f}%')
    print(f'Reference tree: {hits1.keys()}')
    print(f'Current tree:   {hits2.keys()}')
