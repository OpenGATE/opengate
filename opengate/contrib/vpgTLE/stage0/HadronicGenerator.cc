  ...
  fPhysicsCase( physicsCase ), fLastHadronicProcess( nullptr ), fPartTable( nullptr )
{
  ...
  //theNeutronHPCapture->BuildPhysicsTable(*G4Neutron::Neutron());
  ...
  //G4VCrossSectionDataSet* theNeutronHPCapturedata = new G4NeutronHPCaptureData;
  ...
  fProcessMapHP.insert( ProcessPair( G4Proton::Definition(), theProtonInelasticProcess ) );

  G4HadronicProcess* theNeutronHPInelasticProcess =
    new G4HadronInelasticProcess( "NeutronHPInelastic", G4Neutron::Definition() );
  fProcessMapHP.insert( ProcessPair( G4Neutron::Definition(), theNeutronHPInelasticProcess ) );
  ...
  fProcessMapHP.insert( ProcessPair( G4Deuteron::Definition(), theDeuteronInelasticProcess ) );
  ...
  fProcessMapHP.insert( ProcessPair( G4Triton::Definition(), theTritonInelasticProcess ) );
  ...
  fProcessMapHP.insert( ProcessPair( G4He3::Definition(), theHe3InelasticProcess ) );
  ...
  fProcessMapHP.insert( ProcessPair( G4Alpha::Definition(), theAlphaInelasticProcess ) );
  ...
  fProcessMapHP.insert( ProcessPair( G4GenericIon::Definition(), theIonInelasticProcess ) );
...
G4int HadronicGenerator::GenerateInteraction(G4ParticleDefinition* projectileDefinition, G4Material* targetMaterial,
    ...
    G4int numCollisions = 1e1;  //***LOOKHERE***  NUMBER OF COLLISIONS
    G4double rnd1(0);
  ...
  if (projectileDefinition == G4Neutron::Neutron() && projectileEnergy < 20 * MeV && projectileEnergy > 4.4 * MeV){
    // For the HP model, we use the neutron HP inelastic process
    auto mapIndex = fProcessMapHP.find(theProjectileDef);
    if (mapIndex != fProcessMapHP.end()) theProcess = mapIndex->second;
    if (theProcess == nullptr) {
      G4cerr << "ERROR: theProcess is nullptr for neutron HP inelastic !" << G4endl;
    }
  }else{
  ...
  if (theProcess == nullptr) {
    G4cerr << "ERROR: theProcess is nullptr for " << theProjectileDef->GetParticleName() << " inelastic !" << G4endl;
  }
  }
	if (theProcess != nullptr) {
