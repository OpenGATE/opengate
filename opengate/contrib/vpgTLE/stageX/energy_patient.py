...
case = 'try'
# Read input image
itk_image = itk.imread("output/" + dossier + "/optiVPG_tle_patient_0_"+ part + "_e.nii.gz")
...
plt.fill_between(energy_axis, histogram + 3 * inc, histogram - 3 * inc, color = "red", alpha = 0.1, linewidth = 1)
...
plt.savefig("output/" + dossier + "/Energy_" + part + "on_" + case + ".pdf", dpi=300, format="pdf", bbox_inches='tight')
