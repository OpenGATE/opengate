...
class tle_sim:
    ...
    def find_emission_vector(self, el, root_data): #to find the TH2D in the root_data corresponding to the element el
        if el == "Phosphor" :
            el = "Phosphorus"
    ...
    def density_mat(self, mat, data_way): #read the datas to find the density of the material
    ...
    def liste_el_frac(self, mat,frac_data): # from material, extract a list of element and a list of fraction. Element and its corresponding fraction are stored with the same index in both lists
        if mat[-3] == "_":
            mat = mat[:-3]
        else : 
            mat = mat[:-2]
    ...
    def gamma_mat(self, name, root_data): #construct the GammaZ corresponding to the material
