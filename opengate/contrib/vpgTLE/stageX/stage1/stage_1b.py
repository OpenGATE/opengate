...
sim = simulation(output = output, File_name = File_name, 
                 job_id = 0, number_of_particles = number_of_particles, 
                 visu = False, verbose = False, actor = actor, Erange = Erange)
...
for data in mat:
    name = data[2]
    if neutr : gamma_neutron[name] = tle_simu.gamma_mat(name, data_neutrons)
...
treated_array_p = np.zeros((250, array_p.shape[1], array_p.shape[2], array_p.shape[3]))  # Initialize the treated array for proton

treated_array_n = np.zeros((250, array_p.shape[1], array_p.shape[2], array_p.shape[3]))  # Initialize the treated array for neutron
gamma_array = np.zeros((250, array_p.shape[1], array_p.shape[2], array_p.shape[3]))   # Initialize the treated array for neutron AND proton
...
for x in range(array_p.shape[3]):
    for y in range(array_p.shape[2]):
        ...
        #Get the Hounsfield Unit (HU) value at the current voxel
            ...
            if(UH_array[Z, Y, X] == -3024): #LOOKHERE : conditions of the CT where there is no matter, to be removed if CT different from the exemple case
                #Skip the voxel if HU value is -3024
                continue
            name = tle_simu.voxel_to_mat_name(UH_array[Z, Y, X], tle_simu.voxel_mat)
            histo_E_p = array_p[:, z, y, x]  # Get the energy histogram for the current voxel
            ...
            if (histo_E_p.sum() != 0): #if nothing is detected, the voxel is not treated
                ...
                spectrum_p = np.dot(histo_E_p, Gamma_p) #np.dot : method used for the spectrum computation BUT, can be optimised ???
            ...
            if neutr :
                ...
                if (histo_E_n.sum() != 0):
                    ...
                    spectrum_n = np.dot(histo_E_n, Gamma_n) #np.dot : method used for the spectrum computation BUT, can be optimised ???
