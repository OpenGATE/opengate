...
p_scatter = ax.scatter(
    s, h,  # r and h coordinates
    c=values,  # Color represents the intensity
    marker="o",  # Marker style
    cmap="Reds", 
    s = 8,
    norm=LogNorm(vmin=1e-7, vmax=vmax))  # Normalize color scale
...
cbar_p.set_ticklabels([f"{tick:.0e}" for tick in ticks_p])  # Format ticks as scientific notation
