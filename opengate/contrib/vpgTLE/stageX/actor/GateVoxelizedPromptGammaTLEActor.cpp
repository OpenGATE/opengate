...
void GateVoxelizedPromptGammaTLEActor::SteppingAction(G4Step *step) {
  ...
  if ((fProtonTimeFlag) ||
    ...
    const G4double &pretime =
        step->GetPreStepPoint()->GetGlobalTime() - T0; // ns
    const G4double &posttime =
        step->GetPostStepPoint()->GetGlobalTime() - T0;             // ns
...
void GateVoxelizedPromptGammaTLEActor::SetVector(py::array_t<double> vect_p,
  ...
  fProtonVector =
      std::vector<double>(vect_p.data(), vect_p.data() + vect_p.size());
  fNeutronVector =
      std::vector<double>(vect_n.data(), vect_n.data() + vect_n.size());
}
