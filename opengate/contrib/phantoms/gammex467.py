...
def add_inserts(sim, mother, starting_angle, distance, materials, basename):
    ...
    dangle = math.radians(360. / ninserts)
    angle = math.radians(starting_angle)
    idx = 1
    while idx <= ninserts:
        add_insert(sim, mother, f"{basename}_{idx}", [
            math.cos(angle) * distance * cm,
            math.sin(angle) * distance * cm, 0. * cm
        ], materials[idx - 1])
        angle += dangle
        idx += 1
...
def add_gammex467_phantom(sim, name="gammex467"):
    ...
    add_inserts(sim, phantom, 67.5, 10.82390837, [
        "LN450",
        "LN300",
        "B200",
        "AP6",
    ...
    add_inserts(sim, phantom, 90., 5., [
        "BR12", "CB2_50", "LN300", "Water_Solid", "BRN-SR2", "LV1", "AP6",
        "SB3"
    ], "insert_in")
