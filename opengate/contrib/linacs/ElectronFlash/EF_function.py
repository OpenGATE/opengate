...
def add_tubs(sim, name, mother, rmin, rmax, height, material, translation, material_colors = None):
    """
...
def add_cons(sim, name, mother, rmin1, rmax1, rmin2, rmax2, height, material, translation, material_colors = None):
    """
...
def get_object_zdimension(components, center_z):
    ...
    """
    get_z_bounds = lambda comp: (comp[-1][2] - comp[-3] / 2 + center_z, comp[-1][2] + comp[-3] / 2+center_z)
...
def build_ElectronFlash(sim,center_x=0, center_y=0, center_z=0, material_colors=None):
    ...
    logger.info(f"Building ElectronFlash linac structure with center offset: X={center_x} mm, Y={center_y} mm, Z={center_z} mm")

    components = [
        # (func, name, mother, args..., material, translation)
        (add_tubs,sim, "tit_window", "world", 0, 18.2, 0.055, "Titanium", [0, 0, 0.0275*mm]),
        ...
        (add_tubs,sim, "st_4", "world", 22.5, 37, 73.5, "Tecapeek", [0, 0, 211.005*mm]),
        (add_tubs,sim, "st_5", "world", 37, 130.8, 20, "AluminiumEGS", [0, 0, 211.005*mm]),
    ...
    logger.info(f"Linac structure range: start at {start_z:.3f} mm, end at {end_z:.3f} mm")
    return end_z

def build_passive_collimation(sim, string_name, center_x=0, center_y=0, center_z=0, material_colors=None):
    """
    ...
    allowed_names = ['app100', 'app40', 'app70', 'shaper40', 'mb_40_holes_11', 'mb_40_slit_11']
    if string_name not in allowed_names:
        raise ValueError(f"[ERROR] Unsupported applicator type: '{string_name}'. Allowed applicators are: {allowed_names}")
    ...
    if string_name == 'app100':
        components = [        
            ...
            (add_tubs, sim, "hollow_cyl", "cyl7",  0, 31.5, 10,       "Air",  [0, 0, -16.4*mm]),
            (add_cons, sim, "hollow_cone2","cyl7", 0, 31.5, 0, 50, 32.8, "Air", [0, 0, 5*mm]),
        ...
        components = [        
            (add_tubs, sim, "cyl7",       "world", 0, 72, 42.8,      "PMMA", [0, 0, 21.4]),
            (add_cons, sim, "hollow_cone2","cyl7", 0, 31.5, 0, 20, 42.8, "Air", [0, 0, 0]),
        ...
        components = [  
            (add_tubs, sim, "cyl_mb1", "world", 25.2, 40,  25,      "Tecapeek", [0, 0, -12.5]), #338.155         
    ...
    if string_name == 'shaper40':
        leaf_objects = []
        for name, pos in leaf_defs:
            obj = add_box(sim, name, "world", 25 if "leaf1" in name or "leaf2" in name else 50,
                        50 if "leaf1" in name or "leaf2" in name else 25,
                        3, "Tungsten", offset("world", pos), material_colors=material_colors)
            leaf_objects.append(obj)
        
    if string_name == 'mb_40_holes_11':
        plate_1 = add_box( sim, "plate1", "world", 50  , 50,  2.5,  "Tungsten", offset("world", [0, 0, 1.25])  )
        plate_2 = add_box( sim, "plate2", "world", 40  , 40,  2.5,  "Tungsten", offset("world", [0, 0, 3.75])  )
        
        hole1_1 = add_box(sim, "hole1_1", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[0])
        hole1_2 = add_box(sim, "hole1_2", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[1])
        hole1_3 = add_box(sim, "hole1_3", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[2])
        hole1_4 = add_box(sim, "hole1_4", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[3])
        hole1_5 = add_box(sim, "hole1_5", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[4])
        hole1_6 = add_box(sim, "hole1_6", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[5])
        hole1_7 = add_box(sim, "hole1_7", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[6])
        hole1_8 = add_box(sim, "hole1_8", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[7])
        hole1_9 = add_box(sim, "hole1_9", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[8])
        hole1_10 = add_box(sim, "hole1_10", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[9])
        hole1_11 = add_box(sim, "hole1_11", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[10])
        hole1_12 = add_box(sim, "hole1_12", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[11])
        hole1_13 = add_box(sim, "hole1_13", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[12])
        hole1_14 = add_box(sim, "hole1_14", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[13])
        hole1_15 = add_box(sim, "hole1_15", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[14])
        hole1_16 = add_box(sim, "hole1_16", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[15])
        hole1_17 = add_box(sim, "hole1_17", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[16])
        hole1_18 = add_box(sim, "hole1_18", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[17])
        hole1_19 = add_box(sim, "hole1_19", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[18])
        hole1_20 = add_box(sim, "hole1_20", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[19])
        hole1_21 = add_box(sim, "hole1_21", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[20])
        hole1_22 = add_box(sim, "hole1_22", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[21])
        hole1_23 = add_box(sim, "hole1_23", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[22])
        hole1_24 = add_box(sim, "hole1_24", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[23])
        hole1_25 = add_box(sim, "hole1_25", plate_1, 1*mm, 1*mm, 2.5*mm, "Air", positions[24])
        hole2_1 = add_box(sim, "hole2_1", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[0])
        hole2_2 = add_box(sim, "hole2_2", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[1])
        hole2_3 = add_box(sim, "hole2_3", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[2])
        hole2_4 = add_box(sim, "hole2_4", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[3])
        hole2_5 = add_box(sim, "hole2_5", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[4])
        hole2_6 = add_box(sim, "hole2_6", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[5])
        hole2_7 = add_box(sim, "hole2_7", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[6])
        hole2_8 = add_box(sim, "hole2_8", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[7])
        hole2_9 = add_box(sim, "hole2_9", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[8])
        hole2_10 = add_box(sim, "hole2_10", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[9])
        hole2_11 = add_box(sim, "hole2_11", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[10])
        hole2_12 = add_box(sim, "hole2_12", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[11])
        hole2_13 = add_box(sim, "hole2_13", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[12])
        hole2_14 = add_box(sim, "hole2_14", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[13])
        hole2_15 = add_box(sim, "hole2_15", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[14])
        hole2_16 = add_box(sim, "hole2_16", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[15])
        hole2_17 = add_box(sim, "hole2_17", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[16])
        hole2_18 = add_box(sim, "hole2_18", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[17])
        hole2_19 = add_box(sim, "hole2_19", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[18])
        hole2_20 = add_box(sim, "hole2_20", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[19])
        hole2_21 = add_box(sim, "hole2_21", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[20])
        hole2_22 = add_box(sim, "hole2_22", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[21])
        hole2_23 = add_box(sim, "hole2_23", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[22])
        hole2_24 = add_box(sim, "hole2_24", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[23])
        hole2_25 = add_box(sim, "hole2_25", plate_2, 1*mm, 1*mm, 2.5*mm, "Air", positions[24])
        ...
        plate_1 = add_box( sim, "plate1", "world", 50  , 50,  2.5,  "Tungsten", offset("world", [0, 0, 1.25])  )
        plate_2 = add_box( sim, "plate2", "world", 40  , 40,  2.5,  "Tungsten", offset("world", [0, 0, 3.75])  )
        hole1_1 = add_box(sim, "hole1_1", plate_1, 1*mm, 10*mm, 2.5*mm, "Air", positions[0])
        hole1_2 = add_box(sim, "hole1_2", plate_1, 1*mm, 10*mm, 2.5*mm, "Air", positions[1])
        hole1_3 = add_box(sim, "hole1_3", plate_1, 1*mm, 10*mm, 2.5*mm, "Air", positions[2])
        hole1_4 = add_box(sim, "hole1_4", plate_1, 1*mm, 10*mm, 2.5*mm, "Air", positions[3])
        hole1_5 = add_box(sim, "hole1_5", plate_1, 1*mm, 10*mm, 2.5*mm, "Air", positions[4])
        hole2_1 = add_box(sim, "hole2_1", plate_2, 1*mm, 10*mm, 2.5*mm, "Air", positions[0])
        hole2_2 = add_box(sim, "hole2_2", plate_2, 1*mm, 10*mm, 2.5*mm, "Air", positions[1])
        hole2_3 = add_box(sim, "hole2_3", plate_2, 1*mm, 10*mm, 2.5*mm, "Air", positions[2])
        hole2_4 = add_box(sim, "hole2_4", plate_2, 1*mm, 10*mm, 2.5*mm, "Air", positions[3])
        hole2_5 = add_box(sim, "hole2_5", plate_2, 1*mm, 10*mm, 2.5*mm, "Air", positions[4])
        

    if string_name == 'shaper40':
        start_z, end_z = get_object_zdimension(final_components, center_z)
        logger.info(f"Passive collimator - '{string_name}' - range: start at {start_z:.3f} mm, end at {end_z:.3f} mm")
...
def build_dosephantombox(sim, _string_name, material, center_x=0, center_y=0, center_z=0,
              dimension_x=None, dimension_y=None, dimension_z=None, material_colors=None):
    """
    ...
    if None in (dimension_x, dimension_y, dimension_z):
        raise ValueError("[ERROR] All dimensions (dimension_x, dimension_y, dimension_z) must be specified.")
    ...
    logger.info(f"Box - '{_string_name}' - of material '{material}' built at: "
...
def add_source(sim, number_of_events):
    ...
    source.energy.spectrum_energies, source.energy.spectrum_weights = np.loadtxt(spectrum_path, unpack=True)
    ...
    logger.info(f"Added source with spectrum from '{spectrum_path}', "
...
def set_shaper_aperture(leaf1, leaf2, leaf3, leaf4, aperture_x_mm, aperture_y_mm):
    ...
    if aperture_x_mm > 40 or aperture_y_mm > 40:
        raise ValueError(f"[ERROR] Requested aperture exceeds maximum of 40 mm: "
