...
def add_patient_dependent_linac_box(sim, linac_name):
    cm = g4_units.cm
...
def add_mlc(sim, linac_name, volume_name):

    mm = g4_units.mm
    cm = g4_units.cm
    linac = sim.volume_manager.get_volume(linac_name)
    volume = sim.volume_manager.get_volume(volume_name)
    leaf_height = 90 * mm
    z_linac = linac.size[2]
    ...
    mlc = sim.add_volume("Box", f"{volume_name}_mlc")
    ...
    leaf = mlc_leaf(volume_name)
    leaf.name = f"{volume_name}_leaf"
    sim.volume_manager.add_volume(leaf, f"{volume_name}_leaf")
...
def add_jaw(sim, linac_name, volume_name, side):
    ...
    # Jaws curve tips
    cylindre = volumes.TubsVolume(name=f"{volume_name}_cyl_leaf" + "_" + side)
    cylindre.rmin = 0
...
def add_bottom_shielding(sim, linac_name, bias_volume_name):
    ...
    # steel_hole.color = [0,0,0,0]

    steel_hole = sim.volume_manager.add_volume("Box", "steel_sheet_hole")
    steel_hole.size = [20.5 * cm, 20.5 * cm, 5 * mm]
    steel_hole.mother = steel_sheet
    steel_hole.material = "G4_AIR"

    pb_hole = sim.volume_manager.add_volume("Box", "spb_hole")
    pb_hole.size = [20.5 * cm, 20.5 * cm, 5 * mm]
    pb_hole.mother = inner_pb_ring
    pb_hole.material = "G4_AIR"
