...
def set_cut_on_linac_head(sim,linac_name,particles,cuts):
    ...
    l_volume = [f"{linac_name}_mlc","jaw_box_right","jaw_box_left",f"{linac_name}_primary_collimator",f"{linac_name}_flattening_filter",\
                f"{linac_name}_back_scatter_plate",f"{linac_name}_target_support_top",f"{linac_name}_target_support_bottom"]
    mm = g4_units.mm
...
def add_mlc(sim, linac_name):
    ...
    mlc_bank_rotation = Rotation.from_euler("X", np.arctan(3.25/349.3), degrees=False).as_matrix()
    ...
    for i in range(len(l_center_translation)):
        if i !=0:
            translation_to_apply.append(l_center_translation[i-1][1] + l_center_translation[i][0])

    translation_to_apply = np.array(translation_to_apply) - leaf_width
    translation_to_apply = translation_to_apply[:int(len(translation_to_apply)/2) +1]
    ...
    final_translation_to_apply = np.array((((-np.array(final_translation_to_apply)).tolist() + final_translation_to_apply[::-1])*2))
    for i in range(len(leaves_pos)):
...
def add_jaw(sim, linac_name, side):
    ...
    traps_y_dimensions = [[32 * mm, 32 * mm, 32 * mm],
    ...
    traps_y_positions = [[0,0,0],[jaw_y/2 -16.3*mm - (93.2 - 84.822)*mm - (84.822)/2*mm ,jaw_y/2 -16.3*mm - (93.2 - 87.964)*mm - (87.964)/2*mm,jaw_y/2 -16.3*mm - 93.2/2*mm],[jaw_z/2 - 22*mm  -10*mm/2,jaw_z/2 - 32*mm - 29*mm/2,jaw_z/2 - 32*mm - 29*mm - 16*mm/2]]

    boxes_to_complete_traps_y_dimensions = [[194*mm,200*mm,210*mm],[(109.5 - 84.822)*mm, (109.5 - 87.964)*mm,16.3*mm],[10 * mm, 29 * mm,16*mm]]
    boxes_to_complete_traps_y_positions = [[0,0,0],[jaw_y/2 - (109.5 - 84.822)/2*mm, jaw_y/2 - (109.5 - 87.964)/2*mm, jaw_y/2 - 16.3/2*mm],[jaw_z/2 - 22*mm  -10*mm/2,jaw_z/2 - 32*mm - 29*mm/2, jaw_z/2 - 32*mm - 29*mm - 16*mm/2]]
    middle_trap_dimension = [[32*mm,32*mm],[73.2*mm,73.2*mm],[45*mm,45*mm]]
    middle_trap_position = [0,jaw_y/2 - 109.5*mm - 73.2*mm/2,jaw_z/2 - 32*mm - 45*mm/2]
    ...
    for i in range(1,len(traps_y_positions)):
        ...
        traps.translation =[traps_y_positions[0][i],traps_y_positions[1][i],traps_y_positions[2][i]]
    ...
    for i in range(3):
        ...
        box.size = [boxes_to_complete_traps_y_dimensions[0][i],boxes_to_complete_traps_y_dimensions[1][i],boxes_to_complete_traps_y_dimensions[2][i]]
        box.translation = [boxes_to_complete_traps_y_positions[0][i], boxes_to_complete_traps_y_positions[1][i], boxes_to_complete_traps_y_positions[2][i]]
        box.mother = jaw_box.name
        the_box = intersect_volumes(box,cylindre,[0, boxes_to_complete_traps_y_dimensions[1][i]/2  -135 * mm,jaw_z/2 - 35*mm  - boxes_to_complete_traps_y_positions[2][i]],rot_cyl)
    ...
    top_jaw_box = intersect_volumes(top_jaw_box, cylindre, [0, top_jaw_box.size[1]/ 2 - 135 * mm,
                                                jaw_z / 2 - 35 * mm - top_jaw_box.translation[2]],rot_cyl)
    dimension_corner = np.array(
        [np.sqrt(22 ** 2 + 6.9 ** 2) * mm, np.sqrt(22 ** 2 + 6.9 ** 2) * 6.9 / 22 * mm, 22 * mm + 10*um])

    for i in range(2) :
        corner_to_remove = volumes.BoxVolume(name="corner_to_remove_" + side +"_" + str(i))
        ...
        translation_to_apply = np.array([194*mm/2 - corner_to_remove.size[0]/2,-jaw_y/2 - corner_to_remove.size[1]/2,0])
        t = gate.geometry.utility.get_translation_from_rotation_with_center(rot_for_corner,
                                                                            [+corner_to_remove.size[0] / 2,
                                                                             -corner_to_remove.size[1] / 2, 0])
        if i == 1:
        ...
        top_jaw_box = subtract_volumes(top_jaw_box, corner_to_remove, translation_to_apply,rot_for_corner.as_matrix())
    ...
    below_top_jaw_box.translation = [0, jaw_y/2 - (109.5 - 84.822) * mm - (jaw_y -(109.5 - 84.822))/2*mm, jaw_z / 2 - 22 * mm  -10*mm/ 2]
    dimension_corner = np.array(
        [np.sqrt(22 ** 2 + 6.9 ** 2) * mm, np.sqrt(22 ** 2 + 6.9 ** 2) * 6.9 / 22 * mm, 10 * mm +10*um])
    for i in range(2) :
        corner_to_remove = volumes.BoxVolume(name="corner_to_remove_" + side +"_"+ str(i + 2))
        corner_to_remove.size = dimension_corner
        angle_for_corner = np.arctan(6.9 / 22) * 180 / np.pi
        if i == 1:
            angle_for_corner = - angle_for_corner
        rot_for_corner = Rotation.from_euler("z", -angle_for_corner, degrees=True)
        translation_to_apply = np.array([194*mm/2 - corner_to_remove.size[0]/2,-(jaw_y - (109.5 - 84.822)*mm)/2 - corner_to_remove.size[1]/2,0])
        t = gate.geometry.utility.get_translation_from_rotation_with_center(rot_for_corner,
                                                                            [+corner_to_remove.size[0] / 2,
                                                                             -corner_to_remove.size[1] / 2, 0])
        if i == 1:
            ...
            t = gate.geometry.utility.get_translation_from_rotation_with_center(rot_for_corner,
                                                                                [-corner_to_remove.size[0] / 2,
                                                                                 -corner_to_remove.size[1] / 2, 0])
        translation_to_apply += t
        below_top_jaw_box = subtract_volumes(below_top_jaw_box, corner_to_remove, translation_to_apply,rot_for_corner.as_matrix())
    ...
    below_top_jaw_box = subtract_volumes(below_top_jaw_box, left_edge_box_to_remove,[- 154 * mm / 2 + size_box[0] / 2, - below_top_jaw_box_size[1] / 2 + 80 *mm + size_box[1]/2,0] ,np.identity(3))
    sim.volume_manager.add_volume(below_top_jaw_box, 'truncated_' + below_top_jaw_box.name)
    ...
    for i in range(2) :
        trap_to_complete_jaw = sim.volume_manager.add_volume("Trap", "trap_to_complete_jaw_" + side+"_" +str(i))
        alpha = np.arctan(0.5 * 20 / 20.8775)
        if i == 1 :
            alpha = -alpha
        trap_g4_param(trap_to_complete_jaw,
                      1* nm / 2,
                      20*mm/2,
                      1* nm / 2,
                      20*mm/2,
        ...
        if i == 0 :
            trap_to_complete_jaw.translation = [jaw_x/2 -  (210-194)/2*mm - 20/2 * mm - 10/2 *mm + 1*nm,jaw_y/2 - (109.5 - 84.822) * mm - 20.8775/2*mm, jaw_z/2 - 22*mm  -10*mm/2]
        if i == 1 :
            trap_to_complete_jaw.translation = [- (jaw_x / 2 - (210 - 194) / 2 * mm - 20 / 2 * mm - 10 / 2 * mm + 1 * nm),
    ...
    for i in range(2):
        traps_edge_jaw = sim.volume_manager.add_volume("Trap", "traps_edge_jaw_" + side+"_" +str(i))
        alpha =  - np.arctan(0.5*6.324/10.64)
        if i == 1:
            alpha = - alpha

        # alpha = 0
        trap_g4_param(traps_edge_jaw,
                      (1* nm)/ 2,
                      (57.5*mm)/2,
                      (1* nm)/ 2 ,
                      (57.5*mm)/2,
        ...
        traps_edge_jaw.translation = np.array([jaw_x/2 - (210 - 194) / 2 * mm - 20 *mm - 0.5 * 21.28*mm* np.tan(alpha) - 57.5*mm/2,jaw_y/2 - (109.5 - 84.822) * mm - 20.8775*mm - 21.28*mm/2, jaw_z/2 - 32*mm - 15*mm/2])
        if i == 1:
            traps_edge_jaw.translation[0] =  - jaw_x/2 + (210 - 194) / 2 * mm + 20 *mm - 0.5 * 21.28*mm* np.tan(alpha) + 57.5*mm/2
        rot_angle = np.arctan(87.964/((200-32)/2))
        if i == 1:
            rot_angle =  - rot_angle
        rot =  Rotation.from_euler("z", rot_angle*180/np.pi, degrees=True)
        rot_inversion = Rotation.from_euler("xz", [180,-180 + rot_angle*180/np.pi], degrees=True)
        vec_translation_for_rotation  = np.array([57.5*mm/2 + 0.5 * 21.28*mm* np.tan(alpha) , 21.28*mm/2,0])
        if i == 1 :
            vec_translation_for_rotation[0] = - 57.5*mm/2 + 0.5 * 21.28*mm* np.tan(alpha)
        ...
        if i == 1 :
            translation_for_overlap_correction[0] = - translation_for_overlap_correction[0]
    ...
    band_box_to_add_left_side = sim.volume_manager.add_volume("Box",'band_box_to_add_left_side_' + side)
    band_box_to_add_left_side.size = [20*mm,10*mm,10*mm]
    band_box_to_add_left_side.translation = [- 194*mm/2 + band_box_to_add_left_side.size[0]/2,-jaw_y/2 + 70*mm + band_box_to_add_left_side.size[1]/2,jaw_z/2 - 22*mm - 10*mm/2]
...
def retrieve_offset_data_to_apply(offset_type: object, collimation_type: object) -> object:
    mlc_offset_thales_to_radiation_field = [
        [-149.19, -144.56, -135.25, -125.90, -116.51, -107.06, -97.57, -88.03, -78.45, -68.81, -59.12, -49.39, -39.61,
         -29.78, \
         -19.9, -9.98, 0, 10.02, 20.10, 30.22, 40.39, 50.61, 60.88, 71.19, 81.55, 91.97, 102.43, 112.94, 123.49, 134.1,
         144.75, \
         155.44, 166.19, 176.98, 187.92, 198.71, 209.64], \
        [-5.53, -5.17, -4.47, -3.82, -3.22, -2.67, -2.16, -1.70, -1.29, -0.93, -0.61, -0.35, -0.13, 0.04, \
         0.16, 0.23, 0.25, 0.23, 0.15, 0.03, -0.14, -0.36, -0.63, -0.94, -1.31, -1.72, -2.18, -2.69, -3.25, \
         -3.85, -4.51, -5.20, -5.95, -6.74, -7.58, -8.47, -9.4]]

    jaws_offset_thales_to_radiation_field = [[-127.57,-117.93,-108.26,-98.56,-88.83,-79.08,-69.29,-59.48,-49.64,-39.77,-29.87,\
                                   -19.94,-9.99,-0,10.01,20.06,30.13,40.23,50.36,60.52,70.71,80.92,91.17,101.44,111.74\
                                      ,122.07,132.43,142.82,153.23,163.68,174.15,184.64,195.17,205.72],\
                                  [-2.18,-1.82,-1.49,-1.19,-0.92,-0.68,-0.46,-0.28,-0.12,0.01,0.11,0.18,\
                                   0.23,0.24,0.22,0.18,0.11,0.01,-0.13,-0.28,-0.47,-0.69,-0.93,-1.21,-1.51,\
                                   -1.84,-2.20,-2.59,-3,-3.44,-3.91,-4.41,-4.94,-5.49]]

    mlc_offset_light_field_to_radiation_field = [[-149.19,-144.56,-135.25,-125.90,-116.51,-107.06,-97.57,-88.03,-78.45,-68.81,-59.12,-49.39,-39.61,-29.78,\
                                  -19.9,-9.98,0,10.02,20.10,30.22,40.39,50.61,60.88,71.19,81.55,91.97,102.43,112.94,123.49,134.1,144.75,\
                                  155.44,166.19,176.98,187.92,198.71,209.64],\
                                  [0.28,0.28,0.28,0.27,0.27,0.27,0.27,0.27,0.27,\
                                   0.26,0.26,0.26,0.26,0.26,0.26,0.25,0.25,0.25,0.25,0.25,0.25,0.25,0.25,0.25,0.25,0.25,0.24,0.24,0.24,0.24,0.24,0.24,0.24,0.24,0.24,0.24,0.24]]

    jaws_offset_light_field_to_radiation_field = [[-127.57,-117.93,-108.26,-98.56,-88.83,-79.08,-69.29,-59.48,-49.64,-39.77,-29.87,\
                                   -19.94,-9.99,-0,10.01,20.06,30.13,40.23,50.36,60.52,70.71,80.92,91.17,101.44,111.74\
                                      ,122.07,132.43,142.82,153.23,163.68,174.15,184.64,195.17,205.72],
                                  [0.25,0.25,0.25,0.25,0.25,0.25,0.25,0.24,0.24,0.24,0.24,0.24,0.24,0.24,0.24,0.24,0.24\
                                      ,0.24,0.24,0.24,0.24,0.24,0.23,0.23,0.23,0.23,0.23,0.23,0.23,0.23,0.23,0.23,0.23,0.23]]

    if  collimation_type== 'jaw':
        if offset_type == 'from thales':
            return jaws_offset_thales_to_radiation_field[0],jaws_offset_thales_to_radiation_field[1]
        elif offset_type == 'from light field':
            return jaws_offset_light_field_to_radiation_field[0],jaws_offset_light_field_to_radiation_field[1]

    elif collimation_type== 'mlc':
        if offset_type == 'from thales':
            return mlc_offset_thales_to_radiation_field[0],mlc_offset_thales_to_radiation_field[1]
        elif offset_type == 'from light field':
            return mlc_offset_light_field_to_radiation_field[0],mlc_offset_thales_to_radiation_field[1]
...
def define_pos_mlc_jaws_rectangular_field(mlc,x_field, y_field, sad=1000,opposite_leaf_gap=0):
    mm = g4_units.mm
    ...
    mlc_x_aperture = (center_curve_mlc / sad) * (x_field / 2 - f_offset_mlc(x_field / 2))
    jaws_y_aperture = (center_curve_jaws / sad) * (y_field / 2 - f_offset_jaw(y_field / 2))
    ...
    leaves_to_open = leaves_ID[(leaves_position_y >  - (y_position_for_opening + leaf_width/2)) & (leaves_position_y <   y_position_for_opening + leaf_width/2)]
...
def set_rectangular_field(mlc, jaws, x_field, y_field, sad=1000,opposite_leaf_gap = 0):
    pos_x_leaves, pos_y_jaws = define_pos_mlc_jaws_rectangular_field(mlc,x_field, y_field, sad,opposite_leaf_gap)
...
def linac_rotation(sim, linac_name, angles, cp_id="all_cp"):
    ...
    for n in cp_id:
        rot = Rotation.from_euler("YZ", [gantry_angle[n],collimation_angle[n]], degrees=True)
        t = gate.geometry.utility.get_translation_from_rotation_with_center(
...
def mlc_leaves_dynamic_translation(mlc, leaves_position, cp_id="all_cp", sad=1000, only_return_position = False):
    ...
    if cp_id == "all_cp":
        ...
        cp_id = np.arange(0, nb_cp_id, 1)
    for n in cp_id:
        mlc_x_aperture_left = (center_curve_mlc / sad) * (leaves_position[n] - f_offset_mlc(-leaves_position[n]))
        mlc_x_aperture_right = (center_curve_mlc / sad) * (leaves_position[n] - f_offset_mlc(leaves_position[n]))
        mlc_x_aperture = np.array(mlc_x_aperture_left[:80].tolist() + mlc_x_aperture_right[80:].tolist())
