...
def list_of_beam_sequence_ID(file):
    ...
    for i in range(nb_of_beam_seq):
        number_of_mu = retrieve_value_in_DICOM_RTPlan(patient_info.value[0], [[('0x300C', '0x0004'), i],
                                                                          [('0x300A', '0x0086'), None]])
...
def read(file, cp_id="all_cp", arc_id = None):
    ...
    for beam_seq_id in id_of_beam_sequence:
        ...
        key_list = ["jaws_1", "jaws_2", "rot_angle", "dir_angle", "isocenter", "cumulative weight","collimation angle"]
        l_parameters = [l_jaws_1, l_jaws_2, l_angle, l_dir_angle, l_isocenter, l_dose_weights, l_collimation_angle]
        if cp_id == "all_cp":
        ...
        for id in l_cp_id:
            for i, key in enumerate(key_list):
                if data_set[i][key][id] != None:
                    ...
                    elif key != "rot_angle" and key !="dir_angle" and key !="collimation angle":
            ...
            for i in range(len(data_set[leaves][0])):
                for side_ID in range(2) :
                    if data_set[leaves][0][i]["leaves"][id] != None :
                        leaf_blocks[side_ID].append(data_set[leaves][side_ID][i]["leaves"][id])
                    else :
                        if id != 0 :
                            if data_set[leaves][0][i]["leaves"][id -1] != None :
                                leaf_blocks[side_ID].append(data_set[leaves][side_ID][i]["leaves"][id -1])
                            else :
        ...
        if None in l_collimation_angle :
            ...
            l_collimation_angle = l_collimation_angle[0:-1] + np.diff(l_collimation_angle) / 2
...
def extract_dataset(file,beam_sequence_ID):
    ...
    patient_info = pydicom.dcmread(file)[0x300A, 0x0070]
    number_of_mu = retrieve_value_in_DICOM_RTPlan(patient_info.value[0], [[('0x300C', '0x0004'), beam_sequence_ID],[('0x300A', '0x0086'),None]])
        ...
        if (0x300A, 0x011A) in cp :
            for j in range(len(cp[0x300A, 0x011A].value)):
                ...
                if cp[0x300A, 0x011A].value[j][0x300A, 0x00B8].value == 'ASYMY':
                    ...
                    bool_jaws = True
        if bool_jaws :
            data_set[jaws_1]["jaws_1"].append(retrieve_value_in_DICOM_RTPlan(cp, [[('0x300A', '0x011A'), idx_jaws],[('0x300A', '0x011C'),0]]))
            data_set[jaws_2]["jaws_2"].append(retrieve_value_in_DICOM_RTPlan(cp, [[('0x300A', '0x011A'), idx_jaws], [('0x300A', '0x011C'), 1]]))
        else :
            data_set[jaws_1]["jaws_1"].append(None)
            data_set[jaws_2]["jaws_2"].append(None)

        data_set[angle]["rot_angle"].append(retrieve_value_in_DICOM_RTPlan(cp,[[('0x300A', '0x011E'),None]]))
        data_set[dir_angle]["dir_angle"].append(retrieve_value_in_DICOM_RTPlan(cp,[[('0x300A', '0x011F'),None]]))
        data_set[dose_weights]["cumulative weight"].append(retrieve_value_in_DICOM_RTPlan(cp,[[('0x300C', '0x0050'),0],[('0x300A', '0x010C'),None]]))
        data_set[limiting_device_angle]["collimation angle"].append(retrieve_value_in_DICOM_RTPlan(cp,[[('0x300A', '0x0120'),None]]))
        data_set[isocenter]["isocenter"].append(ds.value[0]['0x300A', '0x012C'].value)
        if bool_MLC :
            for side_ID in range(2):
                for i in range(nb_leaf):
                    data_set[leaves][side_ID][i]["leaves"].append(retrieve_value_in_DICOM_RTPlan(cp,[[('0x300A', '0x011A'),idx_MLC],[('0x300A', '0x011C'),80 * side_ID + i]]))
        else :
            for side_ID in range(2):
