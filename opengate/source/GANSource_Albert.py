...
def load_generator(Generator,name,data_dimensions,number_of_conditions,device):
    ...
    generator_input_dim=z_dim+number_of_conditions #, critic_input_dim = get_input_dimensions(z_dim,data_dimensions,number_of_conditions)
    model = Generator(generator_input_dim,data_dimensions,g_hidden_dim,gen_hidden_layers).to(device)
...
def gener_from_data(GAN,bs,conditions_scaler_file,data_scaler_file,phsp_file):
    ...
    real_labels=conditions_scaler.transform(data_labels[:,1:4])#juste positions#data_labels[:,8:12])
    ...
    #for i in range(int(iterations)):
        ...
        Labels = Label[m:m + cur_batch_size].view(cur_batch_size, number_of_conditions)
    ...
    reparametrisation=np.concatenate((fake_samples_total[:,0:1],post_dir,fake_samples_total[:,4:7]),axis=1)#no need for time
...
class GANSource_Albert:
    ...
    def initialize(self):
        ...
        number_of_conditions=3
        data_dimensions=8
        self.gan=load_generator(Generator,generator_pth_file,data_dimensions,number_of_conditions,device)
        GAN = self.gan
    ...
    def generator(self,source):
        ...
        fake=gener_from_data(GAN,bs,conditions_scaler_file,data_scaler_file,phsp_file)
