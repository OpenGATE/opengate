...
import contrib.spect_ge_nm670 as gate_spect
import itk
import numpy as np
...
def create_spect_simu(sim, paths, number_of_threads=1):
    ...
    spect = gate_spect.add_ge_nm67_spect_head(sim, 'spect', collimator_type=False, debug=False)
    psd = 6.11 * cm
...
def test_spect_hits(sim, paths):
    ...
    is_ok = gate.compare_root2(gate_file, hc_file, "Hits", "Hits", checked_keys,
    ...
    checked_keys = [{'k1': 'globalPosX', 'k2': 'PostPosition_X', 'tol': 1.8, 'scaling': 1},
    ...
    is_ok = gate.compare_root2(gate_file, hc_file, "Singles", "Singles", checked_keys,
    ...
    is_ok = gate.compare_root2(ref_file, hc_file, "Singles", "spectrum", checked_keys,
    ...
    is_ok = gate.compare_root2(gate_file, hc_file, "scatter", "scatter",
    ...
    hc_file = sim.get_actor_user_info("EnergyWindows").output
    checked_keys = [{'k1': 'globalPosX', 'k2': 'PostPosition_X', 'tol': 1.7, 'scaling': 1},
    ...
    is_ok = gate.compare_root2(gate_file, hc_file, "peak140", "peak140",
...
def test_spect_proj(sim, paths, proj):
    ...
    is_ok = gate.assert_images(paths.gate_output / 'projection.mhd',
    ...
    # read image and force change the offset to be similar to old Gate
    is_ok = gate.assert_images(paths.output_ref / 'proj028_ref.mhd',
