...
def gen_cond(n):
    ...
    cond = gate_iec.generate_pos_dir_spheres(spheres_centers, spheres_radius, n_samples, shuffle=True, rs=rs)
    global all_cond
    if all_cond is None:
        all_cond = cond
    else:
...
# gsource.generator = gate.GANSourceDefaultGenerator(gsource)
...
gate_spect.add_simplified_digitizer_Tc99m(sim, 'spect1_crystal', paths.output / 'test038_gan_proj.mhd')
...
scalings = [1] * len(keys)
is_ok = gate.compare_trees(hits1, list(hits1.keys()),
