...
spect1 = gate_spect.add_ge_nm67_spect_head(sim, 'spect1', collimator_type=colli_flag, debug=False)
spect1.translation, spect1.rotation = gate.get_transform_orbiting(p, 'x', 180)

# spect head (debug mode = very small collimator)
spect2 = gate_spect.add_ge_nm67_spect_head(sim, 'spect2', collimator_type=colli_flag, debug=False)
...
gate_spect.add_simplified_digitizer_Tc99m(sim, 'spect1_crystal', paths.output / 'test033_proj_1.mhd')
gate_spect.add_simplified_digitizer_Tc99m(sim, 'spect2_crystal', paths.output / 'test033_proj_2.mhd')
...
is_ok = d < tol
gate.print_test(is_ok, f'Skipped particles ref={ref_skipped}, get {s} -> {d * 100}% vs tol={tol * 100}%')

########################
gate.warning(f'Check stats')
stats_ref = gate.read_stat_file(paths.output_ref / 'test033_stats.txt')
is_ok = gate.assert_stats(stats, stats_ref, 0.05) and is_ok

# compare edep map
gate.warning(f'Check images')
is_ok = gate.assert_images(paths.output_ref / 'test033_proj_1.mhd',
                          paths.output / 'test033_proj_1.mhd',
                          stats, tolerance=45, axis='x') and is_ok
is_ok = gate.assert_images(paths.output_ref / 'test033_proj_2.mhd',
