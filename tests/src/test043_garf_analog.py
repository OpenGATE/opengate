...
# spect head
spect = gate_spect.add_ge_nm67_spect_head(sim, 'spect', collimator_type=colli, debug=ui.visu)
...
is_ok = gate.assert_images(paths.gate_output / 'projection_analog.mhd',
                          fn,
                          stat, tolerance=72, ignore_value=0, axis='x') and is_ok

print()
gate.warning('Tests projection (new)')
is_ok = gate.assert_images(paths.output_ref / 'test043_projection_analog.mhd',
