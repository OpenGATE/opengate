...
def get_libG4processes_path():
    for element in os.listdir(os.path.join(return_site_packages_dir(), "opengate_core.libs")):
        if "libG4processes" in element:
            print(os.path.join(return_site_packages_dir(), "opengate_core.libs", element))
